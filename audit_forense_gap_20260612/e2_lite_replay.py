...
DEPLOY_HOUR = {
    ...
    **{s: "2026-06-12 12:00" for s in ["LTC", "XLM", "ETC", "VET", "FET"]},
}
GEN = {**{s:"G1" for s in ["BTC","ETH","BNB","XRP","TRX"]},
       **{s:"G2" for s in ["ONDO","RENDER","POL","SEI","TAO"]},
       **{s:"G3" for s in ["SOL","DOGE","ADA","BCH","LINK"]},
       **{s:"G4" for s in ["LTC","XLM","ETC","VET","FET"]}}
SYMS = list(DEPLOY_HOUR.keys())
END = pd.Timestamp("2026-06-13 05:00", tz="UTC")

models_dir = str(AUDIT / "postfix_models")
configs_dir = "c:/Users/rixip/combolab/regime_wf"
...
bars = {}
for s in SYMS:
    df = pd.read_parquet(AUDIT / "binance_data" / f"{s}USDT_1h_binance.parquet")
...
def close_trade(out, tr, exit_T, exit_price, reason, s):
    ...
    out.append({"symbol": s, "side": tr["side"], "entry_T": tr["entry_T"], "exit_T": exit_T,
                ...
                "reason_exit": reason, "k_entry": tr["k_entry"], "cfg": tr["cfg"], "gen": GEN.get(s,"?")})
