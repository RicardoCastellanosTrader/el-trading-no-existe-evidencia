...
# ventana extendida. `0` desactiva la feature (comportamiento v5.1 original).
# `1` por default: matches en entry_diff=2h (fuera de entry_tol=1) se
# reclasifican como timing_borderline.
...
def detect_timing_borderline_pairs(no_match_vps, no_match_kernel,
    ...
    VPS<->kernel con entry_diff en ventana extendida.

    Un par es 'timing_borderline' si: mismo symbol + mismo side +
    entry_tol < entry_diff_h <= entry_tol + timing_tolerance.

    Cada kernel trade no matcheado se puede asignar como borderline a UN
    solo VPS (greedy por closest entry_diff). Previene double-counting.

    Args:
      no_match_vps: lista dicts con 'idx', 'symbol', 'side', 'entry_candle'.
      no_match_kernel: lista dicts/objects con 'symbol', 'side', 'entry_ts'.
      entry_tol: tolerancia entry actual (default 1h).
      timing_tolerance: delta EXTRA sobre entry_tol (default 1h). `0` =
        feature desactivada (retorna sets vacios).

    Returns:
      vps_borderline: set de idx de no_match_vps reclasificados.
      kernel_borderline: set de indices de no_match_kernel reclasificados.
      pair_details: dict {vps_idx: (kernel_idx, delta_bars)} para reporte.
    """
    if timing_tolerance <= 0:
        return set(), set(), {}

    # Precomputar entry_candle de kernel trades (una sola vez)
    k_candles = []
    ...
    for nm in no_match_vps:
        ...
        for ki, kt in enumerate(no_match_kernel):
            ...
            # Borderline window: fuera de entry_tol, dentro de entry_tol + timing_tolerance
...
def hypothesis_for_no_match(entry, kind, bingx_cache, binance_cache,
    ...
    A34: si is_timing_borderline=True, retorna 'timing_borderline' con
    prioridad sobre no_match_kernel / no_match_bot (precedencia despues de
    diff_reason y micro_precio, consistente con el spec). Si
    cross_exchange_diff activa 'micro_precio', eso gana (ruido real).
    """
    ...
    if kind == 'kernel_no_vps':
        # A34: borderline aplica tambien para kernel sin VPS (par simetrico)
    ...
    # A34: si no hay ruido cross-exchange pero hay borderline match → reclasificar
    if is_timing_borderline:
        return "timing_borderline"
...
def run_audit(args):
    ...
    # timing_tolerance=0 desactiva feature (comportamiento v5.1 original).
    ...
    for nm in no_match_vps:
        ...
        # A34: flag si este VPS fue reclasificado como timing_borderline
    ...
    for ki, kt in enumerate(no_match_kernel):
        ...
        # A34: flag si este kernel fue par de algun borderline VPS
...
def main():
    ...
    # A34: timing_borderline (hipotesis nueva, post-match reclassificacion)
