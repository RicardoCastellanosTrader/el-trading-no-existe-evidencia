...
- Publica la **rama principal completa del repo de trabajo original** (218 commits, 2026-04 → 2026-07), clonada y filtrada con `git filter-repo --replace-text` / `--replace-message` para eliminar identificadores personales; el árbol de trabajo multi-rama completo (34 ramas, 223 commits) se conserva filtrado fuera del remoto público, con las puntas de las 34 ramas del original ancladas en el sello OpenTimestamps (`prerregistros/ots/head_state_20260706.txt`). Los placeholders `IP_DOMESTICA_REDACTADA`, `IP_VPS_TOKIO_REDACTADA`, `IP_VPS_IRLANDA_REDACTADA`, `AWS_ACCOUNT_REDACTADO`, `INSTANCE_ID_TOKIO_REDACTADO`, `INSTANCE_ID_IRLANDA_REDACTADO` son deliberados y sustituyen literales 1:1 — ningún dato de mercado, resultado ni código fue alterado.
