#!/usr/bin/env python3
"""
audit_fidelity_v5_2.py -- Auditoria de fidelidad v5.2 (extension aditiva de v5.1)

v5.2 = v5.1 + dos features aditivas opcionales:

  Feature A (--deploy-boundaries PATH):
    Segmentacion automatica del dataset por deploy boundaries. Reporta match
    rate + CI95 por segmento. Evita que fixes de deploy contaminen el match
    rate global (problema observado en primer audit 2026-04-21, §12 L.25).

  Feature B (--clustering-check):
    Nueva exclusion 'excluido_clustering_divergente' para trades donde
    cluster_live (SIGNALS_RAW.k) != cluster_post_hoc (classify_bars_gmm).
    Captura divergencia estructural por histeresis GMM en bars borderline
    (caso IMX+GRT del 2026-04-21).

Backward compat: sin flags v5.2 -> comportamiento IDENTICO a v5.1 (la logica
heredada no se toca; las nuevas ramas son condicionales a los flags).

Salidas nuevas:
  - stdout: secciones extra "SEGMENTACION POR DEPLOYS" + "CLUSTERING DIVERGENTE".
  - --json-output PATH opcional: reporte machine-readable con segments.

Ejecutar (modo compat v5.1):
  python audit_fidelity_v5_2.py --since 2026-04-19T17:51:00Z

Ejecutar (modo v5.2 completo):
  python audit_fidelity_v5_2.py --since 2026-04-19T17:51:00Z \\
      --deploy-boundaries deploy_boundaries.json --clustering-check \\
      --json-output audits/audit_v5_2_demo.json
"""
...
EXPECTED_LAB_KERNEL_HASH = "fec1725ebe34f013e7063e0e6b237bb3626a09b01ee6d479be830f294811186f"  # G1.1 Path α 2026-04-28 Sesión 1B
...
def write_report(out_path, ctx):
    ...
    # v5.2: usar EXCL_LABELS_V52 si clustering_check activo para labels completos
    labels_map_exec = EXCL_LABELS_V52 if ctx.get('clustering_check_enabled') else EXCL_LABELS
    n_cat = len(labels_map_exec)
    out(f"N excluidos ({n_cat} categorias):              {ctx['n_excluded']}")
    for code, n in ctx['exclusion_counts'].items():
        out(f"    {code:<45s}: {n}  ({labels_map_exec.get(code, '')})")
    ...
    # ==== SECCION 1.5 (v5.2): SEGMENTACION POR DEPLOYS ====
    if ctx.get('segments_metrics'):
        hr()
        out("SECCION 1.5 (v5.2): SEGMENTACION POR DEPLOY BOUNDARIES")
        hr()
        out(f"{'segmento':<28} {'N_eff':>6} {'OK':>5} {'NONE':>5} "
            f"{'EXCL':>5} {'rate':>7} {'CI95':>20}")
        out("-" * 78)
        for seg_name, m in ctx['segments_metrics'].items():
            rate_s = f"{m['match_rate']*100:.1f}%" if m['n_effective'] > 0 else "N/A"
            if m['n_effective'] > 0:
                ci_s = f"[{m['ci_low']*100:.1f}, {m['ci_high']*100:.1f}]"
            else:
                ci_s = "N/A"
            out(f"{seg_name[:27]:<28} {m['n_effective']:>6} {m['matches_ok']:>5} "
                f"{m['none']:>5} {m['excluded']:>5} {rate_s:>7} {ci_s:>20}")
        out()
        wavg = ctx.get('weighted_avg_match_rate')
        if wavg is not None:
            out(f"Weighted avg match rate (ponderado por N_eff): {wavg*100:.1f}%")
        warnings = ctx.get('discontinuity_warnings') or []
        if warnings:
            out()
            out("DISCONTINUITY WARNINGS (delta >= 20pp entre segmentos consecutivos):")
            for w in warnings:
                out(f"  [WARN] {' -> '.join(w['between'])}: "
                    f"{w['rate_a']*100:.1f}% -> {w['rate_b']*100:.1f}% "
                    f"(delta={w['delta_pp']}pp, N={w['n_a']}->{w['n_b']})")
        else:
            out()
            out("(Sin discontinuidades significativas entre segmentos.)")
        out()
    ...
    # ==== SECCION 2.5 (v5.2 A02): SLIPPAGE DIRECTO ====
    if ctx.get('verbose_slippage_enabled') and ctx.get('slippage_per_match'):
        hr()
        out("SECCION 2.5 (v5.2 A02): SLIPPAGE DIRECTO (fill vs signal desde SIGNALS_RAW.p)")
        hr()
        out(f"{'entry_ts':<20} {'symbol':<12} {'side':<6} "
            f"{'fill_e':>10} {'sig_e':>10} {'%e':>8} "
            f"{'fill_x':>10} {'sig_x':>10} {'%x':>8}")
        out("-" * 78)
        entry_pcts = []
        exit_pcts = []
        for r in ctx['slippage_per_match']:
            fe = f"{r['entry_fill']:.4f}" if r['entry_fill'] is not None else "-"
            se = f"{r['entry_signal']:.4f}" if r['entry_signal'] is not None else "-"
            fx = f"{r['exit_fill']:.4f}" if r['exit_fill'] is not None else "-"
            sx = f"{r['exit_signal']:.4f}" if r['exit_signal'] is not None else "-"
            pe = f"{r['entry_slip_pct']:+.3f}" if r['entry_slip_pct'] is not None else "-"
            px = f"{r['exit_slip_pct']:+.3f}" if r['exit_slip_pct'] is not None else "-"
            if r['entry_slip_pct'] is not None:
                entry_pcts.append(r['entry_slip_pct'])
            if r['exit_slip_pct'] is not None:
                exit_pcts.append(r['exit_slip_pct'])
            out(f"{_fmt_ts(r['ts']):<20} {r['symbol']:<12} {r['side']:<6} "
                f"{fe:>10} {se:>10} {pe:>8} "
                f"{fx:>10} {sx:>10} {px:>8}")
        out()
        if entry_pcts:
            mean_e = sum(entry_pcts) / len(entry_pcts)
            max_abs_e = max(abs(v) for v in entry_pcts)
            out(f"Entry slippage: N={len(entry_pcts)}, mean={mean_e:+.3f}%, "
                f"max|.|={max_abs_e:.3f}%")
        if exit_pcts:
            mean_x = sum(exit_pcts) / len(exit_pcts)
            max_abs_x = max(abs(v) for v in exit_pcts)
            out(f"Exit slippage:  N={len(exit_pcts)}, mean={mean_x:+.3f}%, "
                f"max|.|={max_abs_x:.3f}%")
        out()
        out("Convencion: positivo = favorable al trader, negativo = adverso.")
        out("USDT omitido en esta seccion por bug pre-existente size_usdt=0 en CSV.")
        out()
    ...
    hr()
    # v5.2: si clustering_check activo, usar EXCL_LABELS_V52 (incluye exclusion #6)
    labels_map = EXCL_LABELS_V52 if ctx.get('clustering_check_enabled') else EXCL_LABELS
    for code, label in labels_map.items():
        items = ctx['exclusion_items'].get(code, [])
        out(f"[{code}] -- {label}  ({len(items)})")
        for it in items:
            extra_bits = []
            extra_keys = ('diff_pct', 'dd_mult', 'reconcile_cycle', 'cluster',
                          'reason_exit', 'flag', 'cluster_live', 'cluster_post_hoc')
            detail = it.get('detail') or {}
            if isinstance(detail, dict):
                for k in extra_keys:
                    if k in detail:
                        extra_bits.append(f"{k}={detail[k]}")
            extra = " ".join(extra_bits)
            out(f"    {_fmt_ts(it['ts']):<20} {it['symbol']:<12} {it['side']:<6} {extra}")
        out()
    # v5.2 Feature B: report skipped clustering checks
    skipped = ctx.get('clustering_skipped_warnings') or []
    if skipped:
        out(f"[CLUSTERING CHECK SKIPPED] {len(skipped)} trades sin check "
            "por missing cluster_live o cluster_post_hoc:")
        for s in skipped[:20]:  # limitar para no saturar reporte
            out(f"    {s['symbol']:<12} entry={s['entry_candle']} "
                f"(live={s['detail'].get('cluster_live')}, "
                f"post_hoc={s['detail'].get('cluster_post_hoc')})")
        if len(skipped) > 20:
            out(f"    ... ({len(skipped) - 20} more)")
        out()
    out()
...
def run_audit(args):
    ...
    # v5.2 Feature B: cache de cluster_labels y timestamps por simbolo para
    # poder calcular cluster_post_hoc al clasificar exclusiones.
    ...
    if not args.dry_run:
        ...
        for symbol in symbols:
            ...
            if cluster_labels is None:
                ...
                continue
            # v5.2: guardar labels y timestamps alineados para Feature B
            cluster_labels_cache[symbol] = cluster_labels
            timestamps_cache[symbol] = df_naive['timestamp'].values
    ...
    # v5.2: EXCL_LABELS_V52 incluye clustering_divergente si --clustering-check
    clustering_check = bool(getattr(args, 'clustering_check', False))
    if clustering_check:
        excl_codes = list(EXCL_LABELS_V52.keys())
        exclusion_counts = {c: 0 for c in excl_codes}
        exclusion_items = {c: [] for c in excl_codes}
    else:
        excl_codes = list(EXCL_LABELS.keys())  # S4: orden canonico v5.1
        exclusion_counts = {c: 0 for c in excl_codes}
        exclusion_items = {c: [] for c in excl_codes}
    ...
    clustering_skipped_warnings = []  # v5.2 Feature B: tracking de skips
    ...
    for idx, row in vps_window.iterrows():
        ...
        # Pasar entry_candle a classify_exclusion (5 exclusiones v5.1)
        ...
        # v5.2 Feature B: si no excluido por las 5 categorias v5.1, probar
        # clustering divergence como sexta categoria.
        if code is None and clustering_check and not args.dry_run:
            if entry_candle is not None:
                is_div, div_detail = check_clustering_divergent(
                    row['symbol'], entry_candle, log_events,
                    cluster_labels_cache, timestamps_cache,
                )
                if is_div:
                    code = EXCL_CLUSTERING
                    detail = div_detail
                elif div_detail and div_detail.get('note') == 'missing_data':
                    # Skip WARNING: tracking pero no excluir.
                    clustering_skipped_warnings.append({
                        'symbol': row['symbol'],
                        'entry_candle': str(entry_candle),
                        'detail': div_detail,
                    })
        ...
        if code is not None:
            ...
            if code not in exclusion_items:
                exclusion_items[code] = []
    ...
    if clustering_skipped_warnings:
        print(f"\n[v5.2 Feature B] {len(clustering_skipped_warnings)} trades sin "
              f"clustering check por missing_data (cluster_live o post_hoc = None)")
    ...
    # v5.2 Feature A: segmentacion por deploy_boundaries
    deploy_boundaries = None
    segments_metrics = None
    discontinuity_warnings_list = []
    bpath = getattr(args, 'deploy_boundaries', None)
    if bpath:
        try:
            deploy_boundaries = load_deploy_boundaries(bpath)
            print(f"\n[v5.2 Feature A] Loaded {len(deploy_boundaries)} boundaries from {bpath}")
        except Exception as e:
            print(f"[v5.2 Feature A] ERROR loading boundaries: {e}", file=sys.stderr)
            deploy_boundaries = None

    if deploy_boundaries:
        effective_end = until_ts or pd.Timestamp(datetime.now(timezone.utc))
        segments = build_segments(deploy_boundaries, since_ts, effective_end)
        # Augmentar trade_rows con nombre de segmento (para reporte)
        for row in trade_rows:
            row['_segment'] = assign_trade_to_segment(row.get('entry_ts'), segments)
        # Computar metricas por segmento preservando el orden
        segments_metrics = {}
        for name, seg_start, seg_end in segments:
            seg_trades = [r for r in trade_rows if r.get('_segment') == name]
            segments_metrics[name] = compute_segment_metrics(seg_trades, matches_by_idx={})
            segments_metrics[name]['period'] = {
                'start': seg_start.isoformat() if hasattr(seg_start, 'isoformat') else str(seg_start),
                'end': seg_end.isoformat() if hasattr(seg_end, 'isoformat') else str(seg_end),
            }
        discontinuity_warnings_list = detect_discontinuity_warnings(segments_metrics)

    # Weighted average match rate (si segments)
    weighted_avg_rate = None
    if segments_metrics:
        total_n_seg = sum(s['n_effective'] for s in segments_metrics.values())
        if total_n_seg > 0:
            weighted_avg_rate = sum(
                s['match_rate'] * s['n_effective'] for s in segments_metrics.values()
            ) / total_n_seg

    # v5.2 A02: slippage per trade usando signal_price_lookup (si --verbose-slippage)
    verbose_slippage = bool(getattr(args, 'verbose_slippage', False))
    slippage_per_match = []
    if verbose_slippage:
        try:
            from signal_price_lookup import (
                get_signal_price, compute_slippage_pct,
            )
        except ImportError as e:
            print(f"[v5.2 A02] signal_price_lookup no disponible: {e}", file=sys.stderr)
            verbose_slippage = False
        else:
            # Para cada trade con entry_candle inferido, extraer signal_entry y signal_exit
            for idx, row in vps_window.iterrows():
                ec = row.get('_entry_candle')
                xc = row.get('cycle_ts')
                sym = row['symbol']
                side = row['side']
                if row.get('_excluded') or ec is None:
                    continue
                sep = get_signal_price(
                    log_events.get('signals_raw', {}), ec, sym,
                    action_hint=('LONG' if side == 'long' else 'SHORT'),
                )
                sxp = get_signal_price(
                    log_events.get('signals_raw', {}), xc, sym,
                    action_hint=('CLOSE_LONG' if side == 'long' else 'CLOSE_SHORT'),
                )
                entry_fill = float(row['entry_price']) if pd.notna(row['entry_price']) else None
                exit_fill = float(row['exit_price']) if pd.notna(row['exit_price']) else None
                entry_slip_pct = None
                exit_slip_pct = None
                if sep is not None and entry_fill is not None:
                    entry_slip_pct = compute_slippage_pct(entry_fill, sep, side)
                if sxp is not None and exit_fill is not None:
                    exit_side = 'short' if side == 'long' else 'long'
                    exit_slip_pct = compute_slippage_pct(exit_fill, sxp, exit_side)
                slippage_per_match.append({
                    'ts': ec,
                    'symbol': sym,
                    'side': side,
    ...
    ctx = {
        ...
        # v5.2 fields
        'audit_version': '5.2' if (deploy_boundaries or clustering_check or verbose_slippage) else '5.1-compat',
        'deploy_boundaries': {k: v.isoformat() for k, v in (deploy_boundaries or {}).items()},
        'segments_metrics': segments_metrics,
        'discontinuity_warnings': discontinuity_warnings_list,
        'weighted_avg_match_rate': weighted_avg_rate,
        'clustering_check_enabled': clustering_check,
        'clustering_skipped_warnings': clustering_skipped_warnings,
        'slippage_per_match': slippage_per_match,
        'verbose_slippage_enabled': verbose_slippage,
    }

    # 12. Escribir reporte
    ts_stamp = now.strftime('%Y%m%d_%H%M')
    report_path = os.path.join(THIS_DIR, f"audit_v5_2_report_{ts_stamp}.txt")
    ascii_path, utf8_path = write_report(report_path, ctx)

    # v5.2: JSON output opcional
    if getattr(args, 'json_output', None):
        ...
        json_payload = {
            ...
            'total': {
                ...
                'matches_ok': n_same,
                'matches_diff': n_diff,
                'n_none': len(no_match_vps),
                'match_rate': (n_matches / n_effective) if n_effective > 0 else None,
                'ci95': [ci_low, ci_high] if n_effective > 0 else None,
            },
            'exclusion_counts': dict(exclusion_counts),
            'segments': segments_metrics,
            'weighted_avg_match_rate': weighted_avg_rate,
            'discontinuity_warnings': discontinuity_warnings_list,
            'clustering_skipped': len(clustering_skipped_warnings),
        }
...
# ============================================================================
# v5.2 EXTENSIONS -- Feature A (segmentation) + Feature B (clustering check)
# ============================================================================

# Feature B: nueva exclusion. Posicion 6 en orden canonico (tras las 5 v5.1).
EXCL_CLUSTERING = 'excluido_clustering_divergente'
EXCL_LABELS_V52 = dict(EXCL_LABELS)
EXCL_LABELS_V52[EXCL_CLUSTERING] = (
    'Cluster live (SIGNALS_RAW.k) != cluster post-hoc (GMM stateless)'
)


def load_deploy_boundaries(path):
    """Carga deploy_boundaries.json y devuelve dict {version: pd.Timestamp UTC}
    ordenado cronologicamente por valor.
    """
    if path is None:
        return None
    with open(path, 'r', encoding='utf-8') as f:
        raw = json.load(f)
    items = []
    for ver, ts_str in raw.items():
        if ver.startswith('_'):
            continue  # metadata fields (e.g. _comment, _source_notes)
        if not isinstance(ts_str, str):
            continue  # skip non-string values (e.g. nested dicts in metadata)
        try:
            ...
            print(f"[v5.2] Skip boundary '{ver}': parse error {e}", file=sys.stderr)
    items.sort(key=lambda x: x[1])
    return dict(items)


def segment_name(prev_ver, curr_ver):
    """Genera nombre de segmento entre boundary previa y actual."""
    if prev_ver is None:
        return f"pre-{curr_ver}"
    return f"{prev_ver} -> {curr_ver}"


def build_segments(boundaries, window_start, window_end):
    """Construye lista de segmentos (name, start_ts, end_ts) dada la ventana
    [window_start, window_end] y boundaries ordenadas cronologicamente.

    Segmento i cubre [boundary_{i}, boundary_{i+1}), clipped a la ventana.
    Ultimo segmento cubre [boundary_last, window_end].
    Si hay trades anteriores al primer boundary, se crea segmento 'pre-<first>'.
    """
    if not boundaries:
        return [('total', window_start, window_end)]

    items = list(boundaries.items())  # ya ordenados por load_deploy_boundaries
    segments = []

    # Segmento 'pre-<first>' si la ventana empieza antes del primer boundary
    first_ver, first_ts = items[0]
    if window_start < first_ts:
        seg_end = min(first_ts, window_end) if window_end else first_ts
        segments.append((f"pre-{first_ver}", window_start, seg_end))

    # Segmentos intermedios
    for i in range(len(items) - 1):
        curr_ver, curr_ts = items[i]
        next_ver, next_ts = items[i + 1]
        seg_start = max(curr_ts, window_start)
        seg_end = min(next_ts, window_end) if window_end else next_ts
        if seg_start < seg_end:
            segments.append((segment_name(curr_ver, next_ver), seg_start, seg_end))

    # Ultimo segmento (desde last boundary hasta window_end o 'now')
    last_ver, last_ts = items[-1]
    last_start = max(last_ts, window_start)
    last_end = window_end or pd.Timestamp(datetime.now(timezone.utc))
    if last_start < last_end:
        segments.append((f"{last_ver} -> now", last_start, last_end))

    return segments


def assign_trade_to_segment(entry_candle, segments):
    """Dado entry_candle (pd.Timestamp UTC) y lista de segmentos
    [(name, start, end)], devuelve el nombre del segmento cuyo rango
    [start, end) contiene entry_candle. None si no cae en ninguno."""
    if entry_candle is None:
        return None
    ec = pd.Timestamp(entry_candle)
    if ec.tzinfo is None:
        ec = ec.tz_localize('UTC')
    for name, start, end in segments:
        if start <= ec < end:
            return name
    # Ultimo segmento: incluir upper bound
    if segments:
        last_name, last_start, last_end = segments[-1]
        if last_start <= ec <= last_end:
            return last_name
    return None


def compute_segment_metrics(segment_trades, matches_by_idx, exclusion_counts_by_idx=None):
    """Para una lista de filas trade (dicts con 'vps_idx', 'match_type', 'exclusion'),
    computa N_effective, matches_ok, match_rate + CI95.

    match_type esperado: 'OK', 'DIFF', 'NONE', 'EXCL', 'NO_EC'.
    N_effective = count(OK + DIFF + NONE), i.e. no-EXCL y no-NO_EC.
    matches_ok = count(OK).
    """
    n_effective = 0
    n_ok = 0
    ...
    for row in segment_trades:
        ...
        if mt == 'EXCL':
            n_excluded += 1
            excl = row.get('exclusion', '')
            if excl:
                excl_counts[excl] = excl_counts.get(excl, 0) + 1
            continue
        if mt == 'NO_EC':
            continue
        n_effective += 1
        if mt == 'OK':
            ...
            n_none += 1
    rate = (n_ok / n_effective) if n_effective > 0 else 0.0
    ci_low, ci_high = wilson_ci_95(n_ok, n_effective) if n_effective > 0 else (0.0, 0.0)
    return {
        'n_effective': n_effective,
        'matches_ok': n_ok,
        'matches_diff': n_diff,
...
def detect_discontinuity_warnings(segments_metrics, min_delta_pp=20.0, min_n=3):
    """Compara match rates consecutivos (con N >= min_n en ambos segmentos)
    y flaggea delta >= min_delta_pp. Devuelve lista de warnings."""
    warnings = []
    names = list(segments_metrics.keys())
    for i in range(len(names) - 1):
        a = segments_metrics[names[i]]
        b = segments_metrics[names[i + 1]]
        if a['n_effective'] < min_n or b['n_effective'] < min_n:
            continue
        delta = abs(a['match_rate'] - b['match_rate']) * 100.0
        if delta >= min_delta_pp:
            warnings.append({
                'between': [names[i], names[i + 1]],
                'rate_a': a['match_rate'],
                'rate_b': b['match_rate'],
...
def get_cluster_live_from_signals_raw(sym, entry_candle, log_events):
    """Lee signals_raw[(entry_candle, sym)]['k'] si disponible.
    Devuelve int o None."""
    if entry_candle is None:
        return None
    sr = log_events.get('signals_raw', {}).get((entry_candle, sym))
...
def get_cluster_post_hoc_at_entry(sym, entry_candle, cluster_labels_cache,
                                    timestamps_cache):
    """Lee cluster_labels[bar_idx] para el bar que coincide con entry_candle.
    Usa cache pre-computado de run_audit (classify_bars_gmm stateless sobre
    toda la data descargada).

    Devuelve int o None si no clasificable.
    """
    if entry_candle is None:
        return None
    labels = cluster_labels_cache.get(sym)
    ts_arr = timestamps_cache.get(sym)
    if labels is None or ts_arr is None or len(ts_arr) == 0:
        return None
    target = pd.Timestamp(entry_candle)
    # ts_arr esta en formato naive; normalizar target
    if target.tzinfo is not None:
        target_naive = target.tz_convert('UTC').tz_localize(None)
    else:
        target_naive = target
    target_np = np.datetime64(target_naive)
    mask = ts_arr <= target_np
    if not mask.any():
        return None
...
def check_clustering_divergent(sym, entry_candle, log_events, cluster_labels_cache,
                                 timestamps_cache):
    """Devuelve (is_divergent: bool, detail: dict|None).
    is_divergent=True solo si ambos clusters (live y post_hoc) son int y difieren.
    Si alguno es None, is_divergent=False (no podemos concluir).
    """
    cluster_live = get_cluster_live_from_signals_raw(sym, entry_candle, log_events)
    cluster_ph = get_cluster_post_hoc_at_entry(sym, entry_candle,
                                                 cluster_labels_cache, timestamps_cache)
    if cluster_live is None or cluster_ph is None:
        return False, {
            'cluster_live': cluster_live,
            'cluster_post_hoc': cluster_ph,
            'note': 'missing_data',
        }
    if cluster_live != cluster_ph:
        return True, {
            'cluster_live': cluster_live,
            'cluster_post_hoc': cluster_ph,
            'entry_candle': str(entry_candle),
...
def main():
    parser = argparse.ArgumentParser(description="audit_fidelity_v5_2 (post-v2.3.2 + seg/clust)")
    ...
    # v5.2 flags (aditivos, sin ellos comportamiento == v5.1)
    parser.add_argument('--deploy-boundaries', type=str, default=None,
                        help="v5.2 Feature A: path a JSON con {version: ISO_timestamp}. "
                             "Genera reporte segmentado por deploy boundaries.")
    parser.add_argument('--clustering-check', action='store_true',
                        help="v5.2 Feature B: excluye trades con cluster_live != cluster_post_hoc.")
    parser.add_argument('--json-output', type=str, default=None,
                        help="v5.2: path opcional para reporte machine-readable JSON.")
