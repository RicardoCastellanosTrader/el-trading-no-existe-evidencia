...
**Última actualización:** 29 Abril 2026 SUB-FRAME 3.A.1 SUCCESSOR ARCHIVED_EMPIRICAL — **Sub-Frame 3.A.1 successor methodology fundamentally REFUTED empíricamente cumulative ETAPA 1 N=6 ONDO (1/6 clusters valid 16.7% rate + Cluster 0 top-100 ρ=-0.032 ≈ 0 + FAIL_GATES Gate A FAIL Gate B PASS) + ETAPA 2 N=8 ONDO (0/8 clusters valid 0% rate CATASTROPHIC + NO_DATA decision + process_symbol returned None) + G.0 cluster sparsity inherent ONDO bar distribution Hypothesis C VALIDATED EMPIRICAL (Hypothesis A top-K + B W4 thresholds REFUTED) + 5 fundamental constraints methodology identified empíricamente (single-symbol single-N effective 0-1 cluster + ONDO BIC priori N=8 analytical-vs-operational gap + Gate A ≥2/3 inherently FAIL + cross-symbol generalization impossible single-sym scope + pattern reproduces Frame 2 Gate C FAIL ranking selectivity within survivor pool unchanged) + §12 L36 35ª/39ª/40ª retrospective calibrated cumulative (factor 37× compute over-estimation N=6 ETAPA 1 + lower-tail prediction "EVEN FEWER valid clusters" 25-35% VALIDATED catastrophic 0/8 + Differential signaling N choice critical FAIL direction VALIDATED) + Sub-fase B.0 4 spec violations resolved (commit `35a1d36` Edit 1-6 GAMMA_1_N_VALUES extension + load_top_k_presets + run_cell signature + CLI flags + presets_dir bug fix `output/'` → `output/production/'` §12 L38 22ª + baseline_json_dir absolute) + tests greenfield 5/5 PASS empíricamente + Parte 0_v2 ONDO BIC sweep N=2..12 ~12s analytical zero compute knee N=8 detected (vs BTC knee N=6) H_AUX_1 UNFAVORABLE detected. Total cumulative compute Sub-fase B 31.5 min (~25 min N=6 + ~6 min N=8) **vs spec ~36.75h = factor 70× overestimated empíricamente catastrophic** (Sub-Frame 3.A.1 successor scope refinement methodology fundamentally REFUTED before scaling). Pattern match Sesión 4.5 Gate C FAIL reproducción empirical robust (Cluster 0 top-100 ρ=-0.032 ≈ cross-9 ρ_mean=-0.026). **Frame 3.A meta-redesign trigger validated robust empíricamente** (D.β BOTH FAIL + D.δ Cluster orphan fundamental issue cross-N) + Sub-Frame 3.A.1 ARCHIVED_EMPIRICAL precedent **applied recursively al successor** — empirical evidence cross-Sesiones forcing methodology meta-redesign disciplinada honest cross-iteraciones consolidated robust. §12 L34 cross-43ª aplicación recursiva al own Sub-Frame 3.A.1 successor methodology validity applied honest disciplinada. §12 L37 V2 cumulative reframed permanente catastrophic VALIDATED. §12 L38 cross-25ª aplicaciones recursivas validated robust (Sub-fase B.0 4 spec violations + Parte 0_v2 BIC priori UNFAVORABLE + B.1 + B.2 empirical + G.0 Hypothesis C VALIDATED). Branch `sesion-2-frame3a-smoke-ondo-comparative-n6-n8` preserved git history trazabilidad — code potentially reusable Frame 3.A meta-redesign IF refined approach compatible. Sub-Frames 3.A.2 (γ.4 toxic_tail) + 3.A.3 (γ.3+γ.5) + 3.A.4 (γ.6 HMM CONDITIONAL) STATUS pending Frame 3.A meta-redesign re-evaluable post-meta-redesign sesión. Reciclaje launch new calendar TBD post-Frame 3.A meta-redesign sesión + validation gates evaluation methodology refined. Bot v2.4.5 invariante uptime 7d 9h+ continuous. Sin tocar `live/*` productivo. Próxima **Frame 3.A meta-redesign sesión PRIMER FOCUS** cuando Ricardo confirme disponibilidad — análoga Sesión 2 Frame 3.A redesign methodology estructuralmente (Parte 1 análisis código real + Parte 2 hipótesis emergentes + Parte 3 reformulación spec + commit monolítico). **Sub-fase H.0 §12 L38 26ª aplicación recursiva 2026-04-29 detectó pattern systematic 100% rate cross-3-cases pendientes memoria persistente cross-conversaciones REFUTED legacy framework references** (perfil de extremos `analyze_train_test.py` §12 L38 24ª + walk-forward second experiment `--opt-size 8000 --fwd-size 2000` §12 L38 26ª-A + Market Regime Clustering ~457 metrics §12 L38 26ª-B) ALL pre-Frame 2/3 reformulation April 22+ — Frame 3.A meta-redesign sesión PRIMER FOCUS validated robust empíricamente cumulative + §12 L38 verification primary source MANDATORY pattern institutional permanente cualquier pendiente cross-conversation memory claim futuro Ricardo. **Sub-fase J.0 Frame 3.A meta-redesign Parte 0 preliminary sub-decisiones Ricardo cumulative substantive cross-conversación nueva hoy 2026-04-29 DONE persistente Header L3 + ROADMAP + §13.4 appendix (NO standalone memory file ephemeral, pattern Sesión 2 Frame 3.A redesign precedent respeted estructural)**: Sub-decisión 1 re-evaluación 6 candidatos γ.1-γ.6 cumulative empirical evidence honest disciplinada (γ.1 GMM cluster N adaptive + γ.2 P_threshold adaptive + γ.5 Z_ATR_BTC cross-symbol feature DEPRECATED standalone empíricamente validated + γ.3 cross-exchange Binance↔BingX consistency H_M6 ARCHIVED ROBUSTLY cluster sparsity inherent makes irrelevante primary methodology refinement + γ.4 Régimen transition handling toxic_tail dynamic default activate SUBJECT cluster sparsity resolution pending Frame 3.A meta-redesign cluster aggregation strategy + **γ.6 HMM/regime-switching PROMOTED PRIORITY HIGH paradigm shift architectural fundamental** addresses cluster sparsity inherent via probabilistic regime persistence + smoother transitions vs hard GMM hard-classification, aligns Sub-Frame 3.A.4 conditional Frame 3.D originally documented, Markov regime persistence paradigm fundamentally different current methodology); Sub-decisión 2 approach preliminary Frame 3.A meta-redesign **Opción (C) Hybrid balanced architectural rethink integrated** (cluster aggregation strategy pool sparse clusters cross-N OR cross-sym + HMM CONDITIONAL paradigm shift probabilistic regime persistence vs hard GMM clustering + validation gates redesign criterio empirical adapted cluster sparsity inherent + cross-symbol concurrent validation alternative single-symbol single-N) more promising than Opción (A) single-issue refinement validation gates only insufficient OR Opción (B) full architectural rethink HMM + everything redesigned scope catastrophic; §12 L34 cross-45ª aplicación recursiva al own 6 candidatos γ.1-γ.6 cumulative empirical evidence honest disciplinada applied + §12 L36 42ª aplicación prophilactic predicción Frame 3.A meta-redesign sesión PRIMER FOCUS approach preliminary Opción (C) Hybrid balanced more promising standalone calibration retrospective post-sesión pendiente. **Sub-fase K.0 refinamiento canónico §0.0 + §13.2 objetivo proyecto correctamente formulado: identificación honesta de configuraciones notables dentro del universo de candidatas (origen Ricardo 2026-04-29 sesión continuación post-Sub-fase J.0, con cuantificación verificada empíricamente 26 bits / 20.9M combos válidas / ~30 presets media / 45 símbolos primary source `master.py CONFIG['symbols']` + `regime_wf/*_specialist_configs.json` ≈ orden 10¹⁰ configuraciones). §12 L38 27ª aplicación recursiva K.0.0 detectó pre-redacción factor 2.6× mismatch combos válidas (claim 8M vs empírico 20.9M) + 3-way symbol divergence (47 lab list / 45 dict + JSONs / 48 CSVs+parquets+joblibs) + colisión slot §0.4 — verificación primary source MANDATORY elevated principio operativo §0.0 foundational. Comentario lab_historico_numba_v8_3.py L77 desactualizado corregido en mismo commit Sub-fase K.0 (coherencia espíritu refinamiento canónico). Memory file dedicado `memory/objetivo_proyecto_identificacion_honesta.md` referencia rápida cross-conversation memory. ROADMAP_PRE_RECICLAJE.md Frame 3.A meta-redesign section actualizado prerequisito lectura §0.0 + §13.2 sesión PRIMER FOCUS Parte 1+2+3. Sub-fase L.0 anexo §0.0 estabilidad estructural regímenes + ciclo operacional posterior + tres caveats honestos (no inmutabilidad / transferencia cross-symbol limitada / frecuencia 1h vs ciclos macro) — articulado por Ricardo sesión continuación post-K.0 a partir de observación "un estudio de clusters a lo largo de 17 años no es susceptible de cambiar a medio plazo"; refuerza estructuralmente "reciclaje definitivo" sentido foundational + conecta cross-symbol concurrent validation eje Opción C Hybrid balanced con histórico corto símbolos ONDO/SEI/APT vs BTC ~9 años; §12 L34 cross-50ª aplicación recursiva al own framing previene malinterpretación "reciclaje definitivo = bot inmutable" cross-Sesiones futuras.**
...
5. **Sesión 3.5 Frame 2** (2026-04-29): R4 dry-run cross-3 launched background — empirical compute DEFERRED §12 L37 V2 cumulative refutado.
6. **Sesión 4 Frame 2** (2026-04-29): Gates A+B+C cross-9 evaluation analytical proxy ✅ DONE — Escenario 1 PARTIAL.
7. **Sesión 4.5 Frame 2** (HOY 2026-04-29): Gate C rigorous analytical refinement ✅ DONE — **FAIL strict criterion (0/9 ρ≥+0.30 sig + 2/9 STRONG NEGATIVE over-fit) → Escenario 3 detectado empíricamente robust → Frame 3 redesign mandatory + reciclaje launch ~2026-05-03 SUSPENDED**.
8. **Frame 3 redesign session subsequent** (~10-30h CC + compute background substantial): arquitectura análisis fundamental edge mejora real architectural changes (NO walk-forward methodology refinement scope). Calendar TBD post-Frame 3 análisis arquitectónica fundamental.
9. **Reciclaje launch new calendar TBD post-Frame 3 redesign analysis**.
10. **Post-reciclaje proyecto dedicado**: R6 audit refactor Path β1 entries-filter (~6-10h CC).
...
### 0.0 Objetivo del proyecto: identificación honesta de configuraciones notables dentro del universo de candidatas

El propósito del proyecto no es mejorar el rendimiento del sistema en abstracto. El sistema —entendido como la lógica de señales y exits del indicador SmartDiv replicado en Python con fidelidad bidireccional preservada— tiene un edge promedio limitado en el régimen de mercado actual. Una configuración típica seleccionada del universo arroja PF aproximado 1.2 cuando se ejecuta operacionalmente con fidelidad 1 (lab simula al bot) y fidelidad 2 (bot replica al lab) preservadas. Eso es propiedad estructural del sistema en este régimen y refleja el comportamiento medio del espacio de configuraciones, no un techo de lo identificable.

El propósito real del proyecto es identificar dentro del universo de candidatas las configuraciones específicas cuyo edge real es notablemente superior al promedio. Por el tamaño del espacio de parámetros —del orden de 10¹⁰ configuraciones cuando se combinan combos válidas del config_id × presets disponibles × símbolos operacionales × regímenes de mercado clasificables— existen necesariamente configuraciones con edge real notable. La cuestión no es si existen (existen por fuerza dada la magnitud del espacio) sino si la metodología es capaz de identificarlas correctamente sin confundirlas con configuraciones que parecen buenas por sobrestimación walk-forward, selection bias o overfitting al período de training.

Bajo este marco, todo el aparato metodológico construido toma su sentido completo:

- Filtro de mesetas y robustez paramétrica (extractor_gemas, ≥60% vecinos rentables): distingue configuraciones estructurales —cuyo edge persiste bajo perturbaciones paramétricas pequeñas— de picos de ruido aislados que parecen rentables solo en su valor exacto.
- M2 fix ranking pf_fwd_ci_low directo + W3 bootstrap pf_fwd CI + W4 thresholds estrictos: corrige mecanismos identificados de sobrestimación walk-forward (M1 muestra fwd pequeña, M2 dilución pf_combined media ponderada) que causaban que el ranking metodológico no predijese pf_fwd real.
- Z_BTC factor inclusion archivado empíricamente (Sub-fase A 2026-04-26): candidato architectural change refutado por evidencia BIC sweep cross-5 altcoins. La metodología no requería este factor para mejorar la identificación dentro del universo.
- Frame 3.A meta-redesign cuatro ejes integrados (Opción C Hybrid balanced, post-Sub-Frame 3.A.1 successor ARCHIVED 2026-04-29): mejoras arquitectónicas a la capacidad de identificación. HMM CONDITIONAL γ.6 captura persistencia temporal de regímenes que GMM independent classification ignora estructuralmente. Cluster aggregation strategy resuelve cluster sparsity inherente detectada empíricamente. Cross-symbol concurrent validation aumenta el sample size efectivo más allá del límite single-symbol. Validation gates redesign ajusta el criterio de aceptación post-cambios arquitectónicos.

**Caveat fundamental honesto**: methodology refinement máximo es condición necesaria pero no suficiente. La metodología honesta produce specialists cuyo pf_fwd refleja fielmente el edge real de la configuración seleccionada. Si la metodología identifica una configuración notable con edge real cercano a 2.0 PF, el bot operacional reproduce ese edge con fidelidad bidireccional preservada. Si la metodología no logra identificar configuraciones notables —por constraints estructurales como cluster sparsity inherente detectados empíricamente Sub-Frame 3.A.1 successor ARCHIVED 2026-04-29— el reciclaje produce specialists típicos del promedio aunque las configuraciones notables existan en el universo.

La pregunta operacionalmente relevante es: **¿cuán bueno es el mejor edge real identificable por la metodología actual dentro del universo de candidatas?** Esa pregunta depende del aparato metodológico, del universo de candidatas existente y de los regímenes presentes en los datos —no del sistema en abstracto ni del régimen de mercado en abstracto.

**Principio operativo derivado**: la verificación primary source contra el código y los datos reales es prerequisito de cualquier afirmación numérica, framing o referencia cross-Sesiones del proyecto. La memoria persistente cross-conversaciones puede arrastrar valores desactualizados o framings mal formulados; la lectura primary source antes de propagar es disciplina foundational, no lección operacional opcional. Este principio está formalizado en §12 Lección 38 con aplicaciones cumulative documentadas y debe leerse como extensión natural de §0.0.

**Estabilidad estructural de regímenes y horizonte foundational del reciclaje**

Una propiedad relevante para el alcance del proyecto es que los regímenes de mercado detectados sobre símbolos con histórico amplio —paradigmáticamente BTC con datos disponibles desde 2017 (~9 años a 1h) o más con datos de timeframe diario extendido— capturan estructura aproximadamente estacionaria a medio plazo. Eventos futuros caen probabilísticamente en clusters ya existentes, no generan clusters nuevos a la frecuencia operacional 1h. Esta propiedad refuerza estructuralmente el marco operativo del reciclaje: las decisiones metodológicas foundational —cómo se identifican configuraciones notables, qué validation gates se aplican, cómo se compone el survivor pool, cómo se asignan a regímenes— se cierran tras Frame 3.A + 3.B + 3.C completo y permanecen válidas durables. El reciclaje launch resultante es definitivo en sentido foundational: no requiere repetirse cada N meses para captar nuevos regímenes, porque los regímenes ya están captados.

Esto NO significa que el bot opere idénticamente para siempre. El ciclo operacional posterior al reciclaje foundational incluye re-evaluación periódica de configuraciones conforme la cola larga del histórico crece, monitoreo de assignment configuración↔régimen actual del mercado en tiempo real, y detección de outliers que sugieran régimen no captado y triggering de re-clustering condicional. Pero el aparato metodológico de selección y validación queda foundational establecido y no se revisa salvo evidencia empírica robusta de cambio estructural a la frecuencia operacional.

Tres caveats honestos sobre los límites de esta estacionariedad:

Primero, estabilidad no es inmutabilidad. Crypto tiene precedentes documentados de cambios de régimen estructurales: pre-2017 (dominancia BTC absoluta), 2017-2020 (emergencia altcoins, DeFi nascent), 2020-2022 (institucional + apalancamiento masivo + colapsos centralizados FTX/LUNA), 2022-presente (post-colapso, regulación, BTC ETF). Es plausible que el espacio de regímenes detectado a 2026 sobre histórico amplio capture todos estos sub-períodos como clusters identificados, pero también es plausible que aparezca un régimen futuro estructuralmente nuevo —ej. transición a mercado predominantemente institucional con dinámicas intra-1h distintas— que no esté representado adecuadamente en el training. El monitoreo operacional descrito arriba es la red de seguridad para este escenario.

Segundo, la propiedad de estacionariedad aplica robustamente a símbolos con histórico amplio (BTC ~9 años a 1h). Aplica menos directamente a símbolos con histórico corto: ONDO con aproximadamente un año, SEI con dos años, APT con tres años. Para esos símbolos los clusters detectados pueden estar capturando patrones idiosincráticos del período disponible, no estructura estacionaria genuina. Esto conecta directamente con la justificación del eje cross-symbol concurrent validation del Frame 3.A meta-redesign Opción C Hybrid balanced: extraer estructura compartida entre símbolos para aumentar el sample size efectivo más allá del límite single-symbol histórico, y para mitigar el riesgo de overfitting a particularidades de períodos cortos.

Tercero, los regímenes detectados a frecuencia 1h capturan dinámicas de volatilidad, tendencia y momentum que sí son aproximadamente estacionarias. Estructura macro multi-año (bull/bear cycles, fases de ciclo crypto) no se captura a 1h —se manifiesta como régimen volatilidad alta o régimen tendencia fuerte sin información sobre el momento del ciclo macro. Esto es propiedad esperable de operar a 1h y es operacionalmente correcto: el bot decide a la frecuencia a la que opera, y los regímenes 1h son la abstracción relevante para esas decisiones, no los ciclos macro multi-año.

Bajo este marco, la frase "reciclaje definitivo" tiene contenido operacional preciso: cierre del aparato metodológico foundational, NO inmutabilidad operacional perpetua. Sesiones futuras deben distinguir entre decisiones foundational (cerradas tras reciclaje launch) y decisiones operacionales (vigentes durante todo el ciclo posterior del bot). Origen: conversación Ricardo 2026-04-29 sesión continuación post-Sub-fase K.0, articulado a partir de la observación "un estudio de clusters a lo largo de 17 años no es susceptible de cambiar a medio plazo" como complemento estructural del refinamiento canónico §0.0.

**Nota de cuantificación verificada empíricamente 2026-04-29**: config_id TF kernel ocupa 26 bits (`decode_config` en `lab_historico_numba_v8_3.py:1202-1212`, bits 0-25), produciendo **20,891,648 combinaciones válidas** tras filtros de validez (Path 1 sin divergencias = 2,048; Path 2 con divergencias = 20,889,600). Presets por símbolo en CSVs producción `output/production/presets_*.csv`: media **30.5** (rango 28-32 sobre 48 CSVs). Símbolos operacionales bot productivo: **45** confirmado primary source `master.py CONFIG['symbols']` + `regime_wf/*_specialist_configs.json` (45 archivos consumidos al arranque del bot VPS Tokio); divergencia 47 (`SYMBOLS` list en `lab_historico_numba_v8_3.py`) y 48 (CSVs producción + parquets `data_cache/` + joblibs `regime_models/`) son artefactos lab/legacy pendientes de cleanup post-reciclaje (TON+PEPE+BONK eliminados del universo operacional BingX Futures, residuos en lab). Regímenes clasificables varían según GMM N priori (típicamente 3-8 clusters cross-símbolo). El cómputo 20.9M × ~30 × 45 × ~5 ≈ 1.4 × 10¹⁰ ilustra el orden de magnitud foundational. Estos números pueden cambiar con refactorings futuros (preset diversity ajustes, símbolos eliminados/añadidos, bits config_id revisados); sesiones futuras deben re-verificar primary source antes de citar valores específicos.

Cualquier sesión futura debe leer este bloque antes de re-interpretar el rendimiento ~1.2 PF del bot operacional o de proponer scope reformulations a Frame 3.A/3.B/3.C. Origen: conversación 2026-04-29 sesión continuación post-Sub-fase J.0. Refinamiento articulado por Ricardo en respuesta a caveat mal formulado en respuestas previas ("edge real estructural marginal mercado potentially independiente methodology refinement") que confundía edge promedio del sistema con edge identificable de la mejor configuración del universo. La formulación correcta articulada por Ricardo: "el problema es encontrar aquellas configuraciones que arrojan resultados notables, lo cual pasa porque lo simulado no sobrestime el rendimiento, y estas configuraciones existen por fuerza ya que tenemos millones de candidatas, y de ahí el filtro de mesetas, vecindad, etc.".
...
**[FRAME 3.A UPDATE 2026-04-29 sesión Sesión 1]**: Sesión 1 Frame 3.A análisis arquitectónica fundamental done — 6 candidatos γ.1-γ.6 evaluation comprehensive + scoring refined + Sub-Frame 3.A roadmap detailed. Walk-forward methodology Frame 2 refinements infrastructure DEPLOYABLE post-Frame 3.A conclusión OR independent reciclaje (escenario F3.3). Frame 3.A ataca **edge real disponible mercado architectural changes** (régimen detection methodology refinement) — NO walk-forward methodology refinement scope. Sub-Frame 3.A.1 priority#1 (combined γ.1+γ.2 H1) trigger documented. Validation gates stricter criterion ≥6/9 stable+POSITIVE per Sub-Frame. Ver §13.4 entrada Sesión 1 Frame 3.A 2026-04-29.

**[FRAME 3.A SUB-FRAME 3.A.1 ARCHIVED_EMPIRICAL + Frame 3.A methodology redesign mandatory 2026-04-28 sesión continuación]**: Sub-Frame 3.A.1 ablation matrix grid sweep methodology REFUTED empíricamente catastrophic factor 20-30× (smoke single cell ~10.7h + ~307 GB; full grid ~900h + ~25 TB). Frame 3.A methodology fundamental redesign mandatory + Sesión 2 Frame 3.A análisis arquitectónica trigger documented. **Caveat permanente §13.2 Frame 3 methodology compute estimates**: empirical smoke + 5-10× safety factor over JSONs-based projections + storage/compute constraints NOT ilimitados — hardware finitos. Pattern institutional consolidated robust cross-Frame 2 → Frame 3 transition reproduced Sub-Frame 3.A.1 internal: empirical evidence forcing methodology redesign analogous Sesión 4.5 Gate C FAIL → Frame 3 redesign mandatory. Refinements arquitectónicos candidatos Sesión 2 Frame 3.A redesign preliminary: smart sampling Bayesian opt + single-symbol single-cluster prototype + reduced kernel scope + JSON metadata-only analytical. Ver §13.4 entrada Sub-Frame 3.A.1 ARCHIVED_EMPIRICAL 2026-04-28.
...
**[FRAME 3.A UPDATE 2026-04-29 sesión Sesión 1]**: Frame 2 CONCLUSION done Sesión 4.5 → Frame 3 redesign mandatory sequential focused (γ) → (β) → (δ) priority order activated post-Sesión 0 Frame 3 metodológica fundamental. **Sesión 1 Frame 3.A análisis arquitectónica fundamental done** (commit Sesión 1 Frame 3.A 2026-04-29): 6 candidatos γ.1-γ.6 evaluation comprehensive Parte 1 + scoring refined Parte 2 + hipótesis emergentes H1+H2+H3 evaluated + Sub-Frame 3.A roadmap detailed (3 Sub-Frames + 1 conditional). **Sub-Frame 3.A.1 priority#1** (combined γ.1 GMM cluster N adaptive + γ.2 P_threshold adaptive H1 architectural coupling) trigger documented + ablation matrix mandatory 4 condiciones + decision rule Opción γ gated-marginal. **Frame 3.A cumulative scope total**: ~28-44h CC + ~50-100h compute background cross-Sub-Frames 3.A.1 + 3.A.2 + 3.A.3 + Sub-Frame 3.A.4 conditional +25-40h CC IF Sub-Frames 3.A.1 + 3.A.2 + 3.A.3 cumulative FAIL Gate C rigorous re-evaluation. **Validation gates analogía Frame 2 stricter criterion**: ≥6/9 stable+POSITIVE (NOT just |ρ|≥0.3 sign consistent) + 0/9 STRONG NEGATIVE + PF empírica cross-9 ≥1.2 baseline real edge. Ver §13.4 entrada Sesión 1 Frame 3.A 2026-04-29.

**[FRAME 3.A SUB-FRAME 3.A.1 ARCHIVED_EMPIRICAL UPDATE 2026-04-28 sesión continuación]**: Sub-Frame 3.A.1 (combined γ.1+γ.2 H1) ARCHIVED_EMPIRICAL post-Sub-fase 3.A.1.2 smoke empirical reality catastrophic refuted spec methodology original factor 20-30× (single cell BTC baseline ~10.7h compute + ~307 GB transient disk; full grid 84 runs ~900h ≈ 37.5 días + ~25 TB cumulative I/O). Decisión Ricardo strategic Opción F: Frame 3.A methodology fundamental redesign mandatory + Sesión 2 Frame 3.A análisis arquitectónica trigger. Sub-Frame 3.A.2 (γ.4) + 3.A.3 (γ.3+γ.5) + 3.A.4 (γ.6 HMM CONDITIONAL) STATUS pending Frame 3.A methodology redesign — re-evaluable post-Sesión 2 Frame 3.A. **Caveat permanente §13.2 Frame 3 methodology compute estimates**: empirical smoke + 5-10× safety factor over JSONs-based projections + storage/compute constraints NOT ilimitados (cognitive + tiempo Ricardo ilimitados, **hardware constraints empíricos disco + GPU + RAM + I/O bandwidth finitos**). Branch `sesion-1-frame-3a1-2-ablation-matrix-background-launch` preserved git history trazabilidad (4eea4ec + 9d3f156) — NO merged main intencionalmente. KILL smoke SIGTERM PID 1791 + cleanup 47 GB disk recovered. Ver §13.4 entrada Sub-Frame 3.A.1 ARCHIVED_EMPIRICAL 2026-04-28.
...
**[REFINAMIENTO CANÓNICO 2026-04-29 — Objetivo del proyecto correctamente formulado: identificación honesta dentro del universo de candidatas]**

La interpretación del rendimiento ~1.2 PF del bot operacional como "edge real estructural marginal del sistema potentially independiente methodology refinement" era framing categórico incorrecto. Conversación Ricardo 2026-04-29 sesión continuación post-Sub-fase J.0 estableció el marco correcto que debe usarse como referencia en sesiones futuras para evitar re-derivar el debate erróneo y para entender correctamente el sentido de Frame 3.A meta-redesign + Frame 3.B + Frame 3.C.

**Marco correcto**: ~1.2 PF refleja el edge promedio de configuración típica del sistema en régimen actual con fidelidad bidireccional preservada. NO refleja límite estructural del edge identificable. Por tamaño del espacio de candidatas (20.9M combos válidas/preset × ~30 presets/símbolo × 45 símbolos × regímenes clasificables ≈ orden 10¹⁰) existen necesariamente configuraciones con edge real notablemente superior. Problema operacional del proyecto: identificar esas configuraciones honestamente, sin sobrestimación walk-forward.

**Implicación para Frame 3.A meta-redesign PRIMER FOCUS**: los cuatro ejes del rethink Opción C Hybrid balanced (HMM CONDITIONAL γ.6 + cluster aggregation + cross-symbol concurrent + validation gates redesign) son mejoras a la capacidad de identificación dentro del universo de candidatas, NO intentos de mejorar el sistema en abstracto. La distinción es foundational y debe preservarse cross-Sesiones.

**Caveat honesto persistente**: methodology refinement es condición necesaria pero no suficiente. Configuraciones notables existen en el universo por fuerza estadística del tamaño del espacio, pero identificarlas honestamente requiere que la metodología supere las constraints estructurales detectadas empíricamente (Sub-Frame 3.A.1 successor ARCHIVED cluster sparsity inherente, Sesión 4.5 Gate C FAIL ranking selectivity within survivor pool unchanged). Frame 3.A meta-redesign es la respuesta arquitectónica a esas constraints.

Este refinamiento es interpretación CANÓNICA del objetivo del proyecto a partir de 2026-04-29. Sesiones futuras deben referenciar este bloque antes de re-interpretar rendimiento ~1.2 PF, antes de proponer scope reformulations a Frame 3.A/3.B/3.C, y antes de articular caveats sobre edge real estructural mercado. Ver **§0.0** (formulación foundational completa con cuantificación verificada empíricamente) + `memory/objetivo_proyecto_identificacion_honesta.md` (referencia rápida cross-conversation memory).

**Origen**: conversación Ricardo 2026-04-29 sesión continuación post-Sub-fase J.0. Caveat mal formulado en respuestas previas ("edge real estructural marginal mercado potentially independiente methodology refinement") corregido por Ricardo articulando: "el problema es encontrar aquellas configuraciones que arrojan resultados notables, lo cual pasa porque lo simulado no sobrestime el rendimiento, y estas configuraciones existen por fuerza ya que tenemos millones de candidatas, y de ahí el filtro de mesetas, vecindad, etc.".

**Anexo §0.0 estabilidad estructural regímenes + ciclo operacional (Sub-fase L.0 2026-04-29)**: el §0.0 incluye anexo sobre estabilidad estructural de regímenes a largo plazo y distinción cierre metodológico foundational vs ciclo operacional posterior, articulado por Ricardo en sesión continuación post-Sub-fase K.0 a partir de la observación "un estudio de clusters a lo largo de 17 años no es susceptible de cambiar a medio plazo". Relevante para entender el alcance del "reciclaje definitivo" en sentido foundational: cierre del aparato metodológico (cómo se identifica, qué validation gates, cómo se compone survivor pool, cómo se asigna régimen) NO inmutabilidad operacional perpetua del bot (re-evaluación periódica de configuraciones + monitoreo assignment + detección outliers + re-clustering condicional permanecen vigentes). Tres caveats honestos articulados: (1) estabilidad ≠ inmutabilidad — crypto tiene precedentes documentados de cambios estructurales multi-año; (2) transferencia cross-symbol limitada para símbolos con histórico corto (ONDO ~1 año / SEI ~2 años / APT ~3 años vs BTC ~9 años) — conecta directamente con la justificación del eje cross-symbol concurrent validation Frame 3.A meta-redesign Opción C Hybrid balanced; (3) frecuencia 1h captura dinámicas estacionarias volatilidad/tendencia/momentum, NO ciclos macro multi-año — los ciclos se manifiestan como régimen volatilidad alta o régimen tendencia fuerte sin información del momento del ciclo, lo cual es operacionalmente correcto para el bot a 1h. Sesiones futuras deben distinguir entre decisiones foundational (cerradas tras reciclaje launch) y decisiones operacionales (vigentes durante todo el ciclo posterior del bot). Ver §0.0 sub-bloque "Estabilidad estructural de regímenes y horizonte foundational del reciclaje" + `memory/objetivo_proyecto_identificacion_honesta.md` sección homónima.
...
**[MEJORA] [EN_ESPERA] Limpieza divergencia símbolos cross-fuentes 47 / 45 / 48 — 2026-04-29**

Contexto: Sub-fase K.0.0bis verificación primary source 2026-04-29 detectó divergencia 3-way en el conteo de símbolos cross-fuentes del proyecto:
- `master.py CONFIG['symbols']` = **45** (autoritativo operacional pipeline orchestrator).
- `regime_wf/*_specialist_configs.json` = **45** (consumidos por bot al arranque, alineado con master).
- `lab_historico_numba_v8_3.py SYMBOLS list` = **47** (lab universe pendiente cleanup).
- `lab_historico_numba_v8_3.py SYMBOL_ZONE_PRESETS dict` = **45** (alineado operacional).
- `output/production/presets_*.csv` = **48** archivos.
- `data_cache/*_1h.parquet` = **48** archivos.
- `regime_models/*.joblib` = **48** archivos.
- `regime_wf/archived/` = **1** (TONUSDT.legacy-2026-04-19).

Diferencia 48 - 45 = 3 consistente con Header L3 "TON, PEPE, BONK eliminados (no disponibles en BingX Futures)" pero residuos no fueron movidos a `archived/` para parquets / joblibs / CSVs / lab SYMBOLS list. SYMBOL_ZONE_PRESETS dict + master.py + JSONs ya están alineados.

Disparo: post-reciclaje próximo (~2026-05-12 a 05-22 calendario tentativo) o post-Frame 3.A meta-redesign sesión PRIMER FOCUS conclusión, según orden ejecución. Cleanup mecánico bajo riesgo (offline tools): mover residuos de TON+PEPE+BONK a `archived/` con sufijo `.legacy-YYYY-MM-DD` siguiendo patrón TONUSDT precedente. Actualizar `SYMBOLS` list lab_historico_numba_v8_3.py de 47 → 45 alineando con SYMBOL_ZONE_PRESETS dict.

Cierre: cuando todas las fuentes converjan en 45 símbolos operacionales con artefactos legacy archivados consistentemente. Smoke §0.8 Nivel A post-cleanup verificará Fidelidad 2 invariante (cleanup es solo organización filesystem + lista lab, no toca brain/kernel).

Referencias: §0.0 nota cuantificación verificada 2026-04-29 + Sub-fase K.0.0bis + Header L3 cumulative cross-conversación 2026-04-29.
...
**APPENDIX 2026-04-29 sesión continuación post-Sub-fase B.3 — Pendientes memoria persistente cross-conversaciones 3/3 systematic legacy framework references VALIDATED EMPÍRICAMENTE**:

Sub-fase H.0 verificación primary source pre-spec pendientes (§12 L38 26ª aplicación recursiva ~20-30 min CC analytical-only) reveló pattern systematic 100% rate cumulative cross-3-cases:

| Pendiente Ricardo claim cross-conversation | §12 L38 # | Status | Empirical evidence legacy framework |
|--------------------------------------------|-----------|--------|-------------------------------------|
| **Caso 1**: "Perfil de extremos" en `analyze_train_test.py` (Sub-fase E) | 24ª | ❌ REFUTED | Script archived `combolab/antiguos de referencia/` March 20 + comboclaude root active (MD5 IDENTICAL); data input `wf_full/dataset_labeled.parquet` does NOT exist; multi-anchor methodology + `is_gem` references `extractor_gemas.py` legacy; ~5-week gap pre-Frame 2/3 reformulation April 22+ |
| **Caso 2**: Walk-forward second experiment `--opt-size 8000 --fwd-size 2000` (Sub-fase H.0.1) | 26ª-A | ❌ **REFUTED** | `--opt-size` + `--fwd-size` flags exist `walk_forward_experiment.py:16` + `walk_forward_v1.py:1185` archived; multi-anchor methodology `compute_anchor_points()`; current `regime_walk_forward.py` (Frame 2/3) uses `--train-ratio` single train/fwd split per cluster — NO `--opt-size`/`--fwd-size` flags |
| **Caso 3**: Market Regime Clustering ~457 metrics (Sub-fase H.0.2) | 26ª-B | ❌ **REFUTED** | "457" only matches `config_id` literals (e.g., 2457036 ONDO C0); "Market Regime Clustering" + "context_metrics" NOT FOUND current Frame 3.A methodology; data legacy `wf_full/` empty (only experiment_config.json 374 bytes March 15) |

**Pattern systemic 100% rate detected empíricamente**: All 3 detected Ricardo cross-conversation pendientes references PRIOR framework legacy (March 15-20 timestamps superseded by Frame 2/3 reformulation April 22+). Cross-conversation memory cross-temporal framework changes invalidates references unless re-validated against current state.

**Implications consolidated honest disciplinada**:
- §12 L34 cross-44ª aplicación recursiva al own pendientes memoria persistente cross-conversaciones: pattern systematic 100% rate VALIDATED — mi memoria persistente cross-conversaciones systemic limitation acknowledged honest disciplinada — NOT reliable source pendientes scope actual current methodology cualquier pendiente.
- §12 L36 41ª aplicación retrospective calibrada empírica honest CRÍTICA: claim "memoria persistente cross-conversaciones registrada" REFUTED 3/3 instances — pattern legacy framework references temporal gap pre-Frame 2/3 reformulation systematic.
- §12 L37 V2 cumulative reframed permanente: pre-spec verification primary source MANDATORY value HIGH cost ~20-30 min CC trivial vs ~4-6h CC × N pendientes futile + potential data regeneration ~hours-days legacy framework restoration saved.
- §12 L38 cross-26ª aplicaciones recursivas validated robust documented permanente: institutional pattern consolidated across 6 cumulative applications cross-conversación nueva 2026-04-29 (21ª/22ª/23ª/24ª/25ª/26ª).

**Mitigation pattern institutional permanent**: §12 L38 verification primary source MANDATORY pattern institutional consolidated robust applied recursively ALL pendientes futures Ricardo claims cross-conversation memory cualquier sesión Claude Code futura.

**Frame 3.A meta-redesign sesión PRÓXIMA primer focus VALIDATED ROBUST EMPÍRICAMENTE CUMULATIVE** post-Sub-Frame 3.A.1 successor ARCHIVED_EMPIRICAL + 3/3 cross-conversation pendientes legacy framework REFUTED. NO alternative parallel direction current methodology current sesión cumulative — Frame 3.A meta-redesign único path forward validated robust.

**APPENDIX 2026-04-29 sesión continuación Sub-fase J.0 — Frame 3.A meta-redesign Parte 0 preliminary sub-decisiones Ricardo cumulative substantive (pattern Sesión 2 Frame 3.A redesign methodology precedent respeted estructural — NO standalone memory file ephemeral, persistente Header L3 + ROADMAP + §13.4 appendix)**:

Pattern Parte 0 preliminary substantive sub-decisiones precedente Sesión 2 Frame 3.A redesign methodology (memory file ephemeral REMOVED post-Parte 3 commit `c6ad5ce`). Por consistency pattern institutional consolidated robust, Parte 0 preliminary persistente Header L3 + ROADMAP + §13.4 appendix instead of standalone memory file redundant.

**Sub-decisión 1 — Re-evaluación 6 candidatos γ.1-γ.6 cumulative empirical evidence honest disciplinada**:

| Candidato | Status post-Sub-Frame 3.A.1 successor ARCHIVED + 3/3 pattern | Razón empírica |
|-----------|-------------------------------------------------------------|----------------|
| **γ.1 GMM cluster N adaptive** | ❌ **DEPRECATED standalone** | BIC priori transferred ONDO UNFAVORABLE H_AUX_1 (knee N=8 vs BTC N=6); cluster sparsity inherent ONDO N=6 1/6 valid + N=8 0/8 catastrophic — N adaptive doesn't resolve cluster sparsity fundamental constraint |
| **γ.2 P_threshold adaptive** | ❌ **DEPRECATED standalone** | FAIL_GATES Cluster 0 top-100 ρ ≈ 0 (Spearman selectivity within survivor pool unchanged) + cluster sparsity inherent makes P_threshold tuning futile within constrained methodology |
| **γ.5 Z_ATR_BTC cross-symbol feature** | ❌ **DEPRECATED standalone** | Depends γ.3 + γ.1 framework superseded; cluster sparsity inherent makes cross-symbol feature addition irrelevante within constrained methodology — cluster aggregation prerequisite |
| **γ.3 Cross-exchange Binance↔BingX consistency H_M6** | ⚠️ **ARCHIVED ROBUSTLY** | Cluster sparsity inherent makes cross-exchange irrelevante as primary methodology refinement; re-evaluable post-meta-redesign cuando cluster aggregation strategy resuelve sparsity issue first |
| **γ.4 Régimen transition handling toxic_tail dynamic default activate** | ⚠️ **SUBJECT cluster sparsity resolution** | Pending Frame 3.A meta-redesign cluster aggregation strategy; toxic_tail dynamic ya implementado Sub-Frame 3.A.1 successor smoke (verified `toxic_tail_mode='dynamic'` ETAPA 1 + 2) but cluster sparsity inherent precludes effective regime transition handling until cluster aggregation strategy resuelve sparsity |
| **γ.6 HMM/regime-switching** | ✅ **PROMOTED PRIORITY HIGH paradigm shift architectural fundamental** | Addresses cluster sparsity inherent via probabilistic regime persistence + smoother transitions vs hard GMM hard-classification; aligns Sub-Frame 3.A.4 conditional Frame 3.D originally documented; Markov regime persistence paradigm fundamentally different current GMM methodology |

**Sub-decisión 2 — Approach preliminary Frame 3.A meta-redesign sesión PRIMER FOCUS Opción (C) Hybrid balanced architectural rethink integrated**:

| Opción | Description | Status |
|--------|-------------|--------|
| (A) Single-issue refinement | Validation gates only redesign (relax thresholds, change criterion) | ❌ REJECTED — insufficient (cluster sparsity inherent constraint not addressed) |
| (B) Full architectural rethink | HMM + everything redesigned standalone | ❌ REJECTED — scope catastrophic (analogous Sub-Frame 3.A.1 ARCHIVED scope catastrophic factor 20-30×) |
| **(C) Hybrid balanced architectural rethink integrated** | **Cluster aggregation strategy + HMM CONDITIONAL + validation gates redesign + cross-symbol concurrent validation** | ✅ **SELECTED preliminary** — addresses cluster sparsity fundamental constraint + paradigm shift selective + tractable scope |

**Components Opción (C) Hybrid balanced**:
1. **Cluster aggregation strategy**: pool sparse clusters cross-N (e.g., aggregate N=8 sparse clusters back to N=4 effective via post-hoc merge) OR cross-sym (cross-symbol cluster pooling para recover validation power); preserves regime detection granularity while resolving cluster sparsity inherent.
2. **HMM CONDITIONAL paradigm shift**: probabilistic regime persistence (Markov chain regime transitions with smooth probabilities) vs hard GMM clustering (discrete bar→cluster classification); reduces "orphan cluster" problem because HMM smooths regime boundaries.
3. **Validation gates redesign**: criterio empirical adapted cluster sparsity inherent — relax cross-cluster requirement (≥2/3 stable+POSITIVE) cuando clusters inherently sparse; introduce alternative metrics like temporal stability OR cross-symbol consistency.
4. **Cross-symbol concurrent validation**: alternative to single-symbol single-N — validate methodology across multiple symbols concurrently (BTC+ONDO+SEI cross-9 baseline) instead of single-symbol single-cluster scope; recovers statistical power lost cluster sparsity inherent.

**Sub-decisión 3 — Header L3 + ROADMAP + §13.4 appendix update sub-decisiones Ricardo Parte 0 preliminary persistente cross-conversation memory mandatory pre-Frame 3.A meta-redesign sesión PRIMER FOCUS PRÓXIMA**:

Pattern Sesión 2 Frame 3.A redesign methodology precedent respeted estructural (memory file ephemeral REMOVED post-Parte 3 commit `c6ad5ce`):
- Header L3 update Sub-decisiones 1-3 documentación cumulative.
- ROADMAP_PRE_RECICLAJE.md update Frame 3.A meta-redesign section + scope refined Parte 1+2+3.
- §13.4 appendix existing Sub-Frame 3.A.1 successor ARCHIVED_EMPIRICAL appendix this section.
- NO standalone memory file ephemeral redundant — pattern institutional persistente Header L3 + ROADMAP + §13.4 appendix.

**§12 disciplinas cross-aplicaciones consolidated robust Sub-fase J.0**:
- §12 L34 cross-45ª aplicación recursiva al own 6 candidatos γ.1-γ.6 cumulative empirical evidence honest disciplinada applied — re-evaluation grounded en cumulative empirical evidence (Sub-Frame 3.A.1 successor ARCHIVED + 3/3 pattern + cluster sparsity Hypothesis C VALIDATED).
- §12 L36 42ª aplicación prophilactic predicción Frame 3.A meta-redesign sesión PRIMER FOCUS approach preliminary Opción (C) Hybrid balanced more promising than (A) single-issue refinement OR (B) full architectural rethink standalone — calibration retrospective post-sesión pendiente.
- §12 L37 V2 cumulative reframed permanente: Frame 3.A meta-redesign sesión PRIMER FOCUS approach preliminary identifiable HOY without empirical compute investment trivial scope analysis-only documentación-only.
- §12 L38 cross-26ª aplicaciones recursivas cumulative validated robust (6 applications cross-conversación nueva 2026-04-29).

**Frame 3.A meta-redesign sesión PRIMER FOCUS scope refined Parte 1+2+3 PRÓXIMA cuando Ricardo confirme**:
...
**Cierre Sub-Frame 3.A.1 successor ARCHIVED_EMPIRICAL**: permanente. Frame 3.A meta-redesign mandatory + sesión PRIMER FOCUS PRÓXIMA cuando Ricardo confirme disponibilidad. Cumulative empirical evidence ETAPA 1 + ETAPA 2 + G.0 cluster sparsity inherent ONDO + 5 fundamental constraints + factor 70× compute over-estimation + pattern Sub-Frame 3.A.1 ARCHIVED precedent applied recursively al successor + 3/3 cross-conversation pendientes legacy framework REFUTED 100% systematic pattern + Parte 0 preliminary sub-decisiones Ricardo cumulative substantive (6 candidatos γ.1-γ.6 re-evaluación + γ.6 HMM PRIORITY HIGH paradigm shift + Opción C Hybrid balanced approach selected) consolidated robust cross-iteraciones cumulative.
...
**Contexto**: Sesión 2 Frame 3.A redesign methodology análoga estructuralmente Sesión 1 Frame 2 análisis arquitectónica fundamental, ejecutada post-Sub-Frame 3.A.1 ARCHIVED_EMPIRICAL trigger 2026-04-28 sesión continuación. Cross-conversación 4 partes (Parte 0 ayer cumulative + Parte 0_v2 nueva + Parte 1 substantive + Parte 2 + Parte 3 commit) cumulative refinements honest disciplinada §12 L34 cross-32ª aplicación recursiva al own scope.

**Re-verify state DONE clean nueva sesión 2026-04-29 (5/5 critical + 2 path drift menores)**:
- R.1 Bot v2.4.5 active VPS uptime 7d 4h 24m post-pause overnight ✓.
- R.2 Main `32456b9` invariante + branch `sesion-1-frame-3a1-2-ablation-matrix-background-launch` (commits `4eea4ec` + `9d3f156`) preserved NO merged main ✓.
- R.3 Memory file `project_sesion_2_frame_3a_parte_0_completed.md` preserved 8783 bytes content comprehensive (7 sub-decisiones + BIC sweep N=2..12 BTC + multi-modal fragility + Sub-Frame 3.A.1 successor scope refined preliminary 8 components) ✓.
- R.4 Optuna v4.8.0 installed ✓.
- R.5 ⚠ 2 path drift menores documental sin impacto: `regime_wf/2026-04-24-smoke/` → actual `regime_wf.backup_smoke_20260423_24/`; `gate_c_rigorous_analytical_results.json` → repo root NOT analysis_scripts/. Recursos accesibles paths alternates.

**§12 L38 18ª aplicación recursiva validated robust nueva sesión**: lectura primaria fuente cross-evidence REAL state preservation post-pause overnight clean. Memory file local + git invariante main + bot uptime continuous + optuna installed verified empíricamente NO asumido cross-conversación.

**Parte 0 ayer Sesión 2 Frame 3.A — 7 sub-decisiones cumulative + BIC sweep BTC empirical** (memory file Parte 0 Concluding):

| # | Sub-decisión | Opción confirmada | Razón |
|---|--------------|-------------------|-------|
| 1 | Single-cluster choice post-M-B2 REFUTED | γ — Single-symbol BTC + heterogeneity post-grid | M-B2 CRÍTICO premise REFUTED (BTC C2 ρ=+0.013 NS noise NOT highest-edge baseline Sesión 4.5) |
| 2 | Bayesian dependency | δ — pip install optuna v4.8.0 | M-A1 resolved, ~50 MB env footprint |
| 3 | Subset presets criterion | η — top-K by pf_fwd_ci_low M2 fix ranking | M-C1 resolved, alineado Gate C metric |
| 4 | BIC sweep N∈{2..8} execute now | Trivial smoke ~70s | M-D1+M-F1 resolved zero compute analytical |
| 5 | Bot v2.4.5 status | Manual SSH Ricardo discretion | M-Bot1 PENDING resuelto re-verify hoy |
| 6 | N_optimo boundary | κ extension N=9,10,12 + λ effective cap N=6 | N=8 boundary detected, walk-forward viability |
| 7 | Post-Paso 1 mixed oscillation | ν — N=6 only confirmed | Multi-modal BIC fragility detected |

**BIC sweep BTC empirical evidence comprehensive (zero compute analytical)**:
- BIC strictly decreasing N=2..12 con knee N=6 BIC=598086 (ΔBIC N=5→6 = -1217 substantial + N=6→7 plateau -161 signature).
- Multi-modal oscillation N=8..12: N=8 bounce-down (596733) + N=9 (596059) + N=10 plateau-uptick (+30) + N=12 boundary unresolved (595003).
- Multi-modal BIC fragility documented honest: GMM `random_state=42` + `n_init=10` reaches different local optima at higher N → N=6 best engineering knee defensible empirical priori.

**Parte 1 hoy substantive — Análisis arquitectónica fundamental cross-5 refinements integrados**:

5 refinements evaluados cross-multi-dimensiones (code surface + compute reduction + statistical power + architectural risk Fidelidad 2 + empirical testability + H1 preserved + priority score):
- Refinement #1 Bayesian TPE optuna smart sampling — MEDIUM/2-3×/ALTA/LOW/ALTA/Y/ALTA.
- Refinement #2 Single-symbol single-pass — LOW/3×/N=100 preserved within sym/LOW/ALTA/Y/ALTA con caveat O1.
- Refinement #3 Subset top-10 pf_fwd_ci_low — MEDIUM/3×/borderline/LOW-MEDIUM/ALTA/Y conditional/MEDIA con caveat O2.
- Refinement #4 N=6 fixed BIC priori — LOW/4× cells/full N=100/LOW/ALTA/PARTIAL (γ.1 NO empirical)/ALTA con caveat O3.
- Refinement #5 γ.1 BIC priori + γ.2 Bayesian hybrid — LOW γ.1 DONE + MEDIUM γ.2/zero γ.1 + Bayesian γ.2/γ.1 BIC + γ.2 Bayesian/LOW/DONE+ALTA/NO H1/ALTA-MEDIA.

**§12 L38 19ª aplicación recursiva — Refinements OVERLAP non-orthogonal detectado**: análisis cross-código real revela Refinements #1+#4+#5 **NOT independent** — overlapping. True orthogonal design space **4-axis decomposition**:
- Axis A: γ.1 N treatment (grid {3,4,5,6} | priori N=6 BIC).
- Axis B: γ.2 P treatment (grid {0.55..0.85} 7 vals | Bayesian continuous).
- Axis C: Symbol scope (cross-3 sym | single BTC | single ONDO).
- Axis D: Preset universe (full ~30 | top-10 pf_fwd_ci_low).

**4 observaciones emergentes Parte 1 honest acknowledged**:
- O1 Single-symbol BTC weakest baseline signal (3 NS noise) — alternativa ONDO better signal detection (STRONG NEG C2 + sig C0).
- O2 Top-10 selection-bias regression to mean within selected universe.
- O3 H1 architectural coupling testability degraded por Refinements #4+#5 (γ.1 priori NO empirical variation).
- O4 Bayesian TPE 1D continuous overkill — targeted grid 7 values simpler reproducible.

**Parte 2 hoy hipótesis emergentes + axis priority order + 4 H_AUX flagged**:

3 H_REDESIGN evaluadas con probabilidades empíricas ground en Sesión 4.5 baseline:
- H1_REDESIGN cross-3 ONDO ρ_mean > +0.30 PASS Gate A: **10-20% probability** (baseline ρ_mean -0.079; lift Δ +0.38 substantial).
- H2_REDESIGN generalization cross-symbol downstream Sub-Frames 3.A.2+3.A.3 conditional H1 PASS.
- H3_REDESIGN decision rule adapted critique: ρ_mean cross-3 NOT robust (single outlier 3× weight inflation) → deprecate ρ_mean primary, ≥2/3 stable+POSITIVE primary + 3/3 sign positive secondary.

**4 H_AUX emergent hipótesis flagged ultrathink (NO listed spec original)**:
- H_AUX_1 N=6 BIC priori BTC → ONDO methodological gap (BIC ayer solo BTC empirical, ONDO N_optimo NOT validated transferred).
- H_AUX_2 toxic_tail dynamic × P_threshold interaction effect (P higher → shorter toxic_tail → más usable bars; confounds edge measurement con bar-budget).
- H_AUX_3 ONDO C2 STRONG NEGATIVE -0.502 deeper mechanism (preset/config space too large for ~1100 train bars/N=6 cluster); probability persists post-refinement **~50-65%**.
- H_AUX_4 Top-10 cross-baseline mismatch (presets ranked baseline N=3 P=0.75 NOT optimal under N=6 P_optimo).

**5 sub-decisiones Ricardo Parte 2 cumulative confirmed**:
- (a) 8 caveats acknowledged honest documented (O1-O4 + H_AUX_1-4) disciplina aspirar-a-lo-mejor literal applied al own scope.
- (b) Parte 0_v2 ONDO BIC sweep ~70s zero compute analytical confirmed (resuelve H_AUX_1).
- (c) Decision rule adapted: deprecate ρ_mean primary + ≥2/3 stable+POSITIVE primary + 3/3 sign positive secondary.
- (d) §12 L36 31ª prophilactic predicción FAIL_GATES ~55-80% most likely outcome ACKNOWLEDGED honest pre-execution.
- (e) **MINIMAL SMOKE 1 cell ONDO N=6 P=0.75 baseline ~3-5h compute pre-execution validation MANDATORY**: pattern Sub-Frame 3.A.1 ARCHIVED_EMPIRICAL precedent (smoke catastrophic refutó full execution pre-investment factor 20-30×). Smoke catches pre-emptive infeasibility cost ~5h vs ~130-370h full investment savings.

**Sub-Frame 3.A.1 successor scope final consolidated post-Parte 0+1+2 + 5 sub-decisiones**:

| Component | Status |
|-----------|--------|
| Axis A γ.1 N | Priori N=6 BIC (DONE Parte 0 BTC + ⚠ Parte 0_v2 ONDO BIC ~70s pendiente sub-decisión b) |
| Axis B γ.2 P | Targeted grid 7 values {0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85} (NOT Bayesian — O4 sub-decisión c implícita) |
| Axis C Symbol | **ONDO single-symbol** (NOT BTC — O1 better signal detection STRONG NEG C2 + sig C0) |
| Axis D Presets | Top-10 pf_fwd_ci_low M2 fix ranking (caveat O2 + H_AUX_4 documented) |
| toxic_tail_mode | dynamic (Sub-fase 3.A.1.0 verified ayer) |
| Pre-execution validation | **Parte 0_v2 ONDO BIC ~70s + Smoke minimal 1 cell ONDO N=6 P=0.75 ~3-5h** |
| Full execution conditional | 7 cells × ONDO × N=6 × top-10 ~26-37h baseline + safety 5-10× = **~130-370h cumulative** |
| Storage cumulative | ~672 GB - 1.05 TB transient (manageable) |

**Reduction vs Sub-Frame 3.A.1 original ~900h compute + ~25 TB**:
- Compute: 2.4-6.9× reduction.
- Storage: 24-37× reduction.

**Validation gates adapted single-symbol ONDO 3 clusters (preserved stricter criterion)**:
- Gate 3.A.1.A: ≥2/3 stable+POSITIVE per ONDO cluster (criterion preserved ≥6/9 → adapted ≥2/3 single-sym).
- Gate 3.A.1.B: 0/3 STRONG NEGATIVE (vs Sesión 4.5 baseline ONDO C2 -0.502 STRONG NEG sig FAIL — Gate B hardest H_AUX_3 deeper mechanism).
- Gate 3.A.1.C: PF empírica ≥1.2 baseline real edge (downstream post-cell Binance 3y validation ~3-5h adicional).

**Decision rule Opción γ gated-marginal adapted single-symbol post-sub-decisión (c)**:
- Primary criterion: ≥2/3 stable+POSITIVE + 3/3 sign positive consistency.
- STRONG ρ_mean cross-3 ONDO > +0.40 sig stable + Primary criterion → early-exit reciclaje launch consideration.
- MARGINAL [+0.30, +0.40] + Primary criterion → proceed Sub-Frame 3.A.2 + 3.A.3 cross-symbol generalization validation.
- WEAK <+0.30 → Sub-Frame 3.A.4 conditional Frame 3.D HMM defer.
- FAIL_GATES → Frame 3.A meta-redesign mandatory (Sub-Frame 3.A.1 ARCHIVED_EMPIRICAL precedent).

**§12 L36 31ª aplicación prophilactic predicción Sub-Frame 3.A.1 successor execution outcome registered explícitamente pre-execution**:

| Outcome | Probability estimate |
|---------|---------------------:|
| **STRONG** ρ_mean > +0.40 + ≥2/3 stable+POS + 0/3 STRONG NEG | 5-10% |
| **MARGINAL** ρ_mean [+0.30, +0.40] + ≥2/3 stable+POS + 0/3 STRONG NEG | 10-15% |
| **WEAK** ρ_mean [+0.10, +0.30] + ≥1/3 stable+POS + 0/3 STRONG NEG | 15-25% |
| **FAIL Gate A** 0/3 stable+POSITIVE | 30-40% |
| **FAIL Gate B** ONDO C2 STRONG NEG persists | 25-40% |
| **Combined FAIL_GATES (any)** | **~55-80%** |

**Most likely outcome**: FAIL_GATES → Sub-Frame 3.A.4 conditional Frame 3.D HMM defer **OR** Frame 3.A meta-redesign mandatory.

**§12 disciplinas cross-aplicaciones consolidated robust cross-Sesión 2 Frame 3.A redesign**:
- §12 L34 cross-32ª aplicación recursiva al own Parte 2 spec: reframe scope honest disciplinada — Opción B refined includes 8 caveats + 4 H_AUX flagged + Parte 0_v2 ONDO BIC + Sub-decisión (e) minimal smoke. Honest scope minimal viable + caveats explícitos > scope minimal viable assumed clean.
- §12 L36 31ª prophilactic predicción registered explícitamente pre-execution outcome FAIL_GATES ~55-80% most likely.
- §12 L37 V2 18ª aplicación: per-cell N=6 cost 1.5-2× higher than N=3 baseline empirical (no anticipado memory file optimistic).
- §12 L37 V2 cumulative consolidated robust permanente: empirical smoke pre-execution validation MANDATORY pattern Sub-Frame 3.A.1 ARCHIVED_EMPIRICAL precedent applied (Sub-decisión e).
- §12 L38 19ª aplicación recursiva validated robust cross-Parte 1: Refinements OVERLAP non-orthogonal reveal 4-axis design space honest decomposition.

**Habilita pre-execution validation cuando Ricardo confirme**:
- Parte 0_v2 ONDO BIC sweep N=2..12 ~70s analytical zero compute (resuelve H_AUX_1).
- Smoke minimal 1 cell ONDO N=6 P=0.75 baseline ~3-5h compute (sub-decisión e mandatory).
- IF Parte 0_v2 + smoke PASS → full execution Sub-Frame 3.A.1 successor 7 cells ~130-370h cumulative cross-multiple sesiones.
- IF Parte 0_v2 + smoke FAIL → scope re-evaluation Frame 3.A meta-redesign pre-investment ~130-370h savings.

**Items §13.3 actualizados**:
- L1296 Frame 3 alcance honesto: Sesión 2 Frame 3.A redesign methodology DONE + Sub-Frame 3.A.1 successor scope refined post-Parte 0+1+2 + 5 sub-decisiones cumulative + Parte 0_v2 + smoke pre-execution validation trigger documented.

**Pattern institutional consolidated robust cross-Frame 2 cumulative + Frame 3 transition + Frame 3.A redesign methodology**: empirical evidence forcing methodology refinement disciplined sequential exploration. §12 L34 + L36 + L37 V2 + L38 cross-aplicaciones consolidated robusto cross-sesiones.

**Bot v2.4.5 invariante uptime 7d 4h+ continuous**. Sin tocar `live/*` productivo (Parte 0+1+2+3 análisis-only documentación-only). Fidelidad 2 invariante por construcción.

**Memory file ayer `project_sesion_2_frame_3a_parte_0_completed.md` REMOVED post-commit Parte 3** (info consolidated §13.4 entry permanent — ephemeral file purpose served).

**Próxima sub-fase Parte 0_v2 ONDO BIC sweep ~70s** cuando Ricardo confirme disponibilidad continuación, antes Smoke minimal 1 cell ONDO N=6 P=0.75 baseline ~3-5h.

**Referencias**:
- Memory file `project_sesion_2_frame_3a_parte_0_completed.md` (Parte 0 ayer cumulative bridge, REMOVED post-commit).
- `analysis_scripts/sub_frame_3a1_gmm_n_pthreshold_grid.py` (664 LOC standalone script ablation matrix grid sweep — Sub-Frame 3.A.1 original ARCHIVED_EMPIRICAL, code base for successor refactor).
- `gate_c_rigorous_analytical_results.json` repo root (Sesión 4.5 baseline cross-9 per-cluster Spearman ρ).
- §13.4 entrada Sub-Frame 3.A.1 ARCHIVED_EMPIRICAL 2026-04-28 (precedente catastrophic factor 20-30×).
- §13.4 entrada Sesión 1 Frame 3.A análisis arquitectónica fundamental 2026-04-29 (commit `4d622e9`).
- §12 cross-aplicaciones consolidated robustas Frame 2 cumulative + Frame 3 redesign methodology.
- ROADMAP_PRE_RECICLAJE.md (Sub-Frame 3.A.1 successor scope refined post-Parte 0+1+2 + Parte 0_v2 + smoke pre-execution validation trigger documented).

**Cierre Sesión 2 Frame 3.A redesign methodology**: permanente. Sub-Frame 3.A.1 successor scope final consolidated post-Parte 0+1+2 + 5 sub-decisiones cumulative + 8 caveats explícitos + 4 H_AUX flagged + §12 L36 31ª prophilactic FAIL_GATES ~55-80% acknowledged + Parte 0_v2 ONDO BIC + Sub-decisión (e) MINIMAL SMOKE pre-execution validation MANDATORY. Próxima Parte 0_v2 ONDO BIC ~70s cuando Ricardo confirme.

---

**[CIERRE METODOLÓGICO] [PRE-RECICLAJE FRAME 3.A SUB-FRAME 3.A.1 ARCHIVED_EMPIRICAL] §12 L37 V2 17ª aplicación catastrophic factor 20-30× refutación spec methodology original + Frame 3.A methodology fundamental redesign mandatory + Sesión 2 Frame 3.A análisis arquitectónica trigger documented — 2026-04-28 sesión continuación post-Sesión 1 Frame 3.A**

**Contexto**: Sub-Frame 3.A.1 (combined γ.1 + γ.2 H1 architectural coupling) ARCHIVED_EMPIRICAL post-Sub-fase 3.A.1.2 smoke empirical reality catastrophic refuted spec methodology original factor 20-30×. Decisión Ricardo strategic Opción F NEW: Sub-Frame 3.A.1 archive empírico + Frame 3.A methodology fundamental redesign mandatory.

**Sub-fases ejecutadas pre-archive**:
- **Sub-fase 3.A.1.0 Parte 0** (commit `4eea4ec` parte): 6 sustantivos detectados pre-implementación (M-A1/A2 architectural code path + M-B2 P_threshold variants + M-C1 compute estimate + M-D1 Sesión 2.5 Opción C reuse + M-F1 determinismo). 5 sub-decisiones Ricardo Opción α + γ + ε + ~12-48h compute calibrated + VPS asumido invariante.
- **Sub-fase 3.A.1.1 implementación standalone script** (commit `4eea4ec`): `analysis_scripts/sub_frame_3a1_gmm_n_pthreshold_grid.py` 664 LOC. Honest correction §12 L34 cross-24ª: 32→31→28 cells unique deduplicated. GMM smoke pre-commit BIC N=6 (598086) < N=3 (607169) sugería empirical priori favorable γ.1.
- **Sub-fase 3.A.1.2 ablation matrix execution background launch** (branch `sesion-1-frame-3a1-2-ablation-matrix-background-launch` commit `9d3f156` preserved): smoke single BTC baseline cell launched bg `b1euswpru` PID 1791 + presets_dir fix `output/production/`. Process killed SIGTERM mid-execution post-empirical refutación catastrophic.
- **Sub-fase 3.A.1.3 tests greenfield paralelo** (commit `9d3f156` preserved branch): 12/12 tests PASS (~3s) — pattern institutional consolidated 39/39 tests cumulative cross-Frame 2 + 3.A.1.

**Empirical reality smoke bg b1euswpru CATASTROPHIC**:
- Timeline: 21:28 launch + 21:38 first variant complete (10 min Numba JIT warmup) + 22:59 variant 8/64 complete = ~1h 30 min para 9/64 variants ~14% progress.
- Per variant después JIT warmup: ~10-11 min steady (CUDA enabled NVIDIA RTX 5070 Laptop GPU).
- Per parts file ~1.6-1.7 GB × 27 parts = ~44 GB transient cumulative actual, scaled extrapolated 64 × 3 × 1.6 GB = **~307 GB transient disk per cell**.
- **Single cell BTC baseline ~10.7h compute + ~307 GB transient disk**.
- **Full grid 84 walk-forward runs: ~900h ≈ 37.5 días continuous compute + ~25 TB cumulative I/O**.
- Spec original Sub-fase 3.A.1.0 ~14-42h compute → realidad ~900h = **factor 20-30× underestimated catastrophic**.

**§12 L37 V2 17ª aplicación cumulative catastrophic captured permanente**:
- Sesión 2.5 R1 DSR factor 10-15× refutado (full pipeline ~30-45h vs spec ~1.5-4.5h).
- Sesión 3 R4 standalone factor 6-8× refutado (~90-225 min vs spec ~10-30 min).
- Sesión 3.5 R4 dry-run bg `bno9hiw92` killed sin completar — empirical compute rejected.
- Sesión 4.5 Gate C analytical zero compute (favorable — JSONs metadata directly).
- Sub-Frame 3.A.1 empirical factor 20-30× refutado catastrophic — magnitud sin precedent Frame 2 cumulative.
- **Pattern recurrent reframed permanente**: Frame 3 compute estimates require **empirical smoke + 5-10× safety factor over JSONs-based projections + storage/compute constraints NOT ilimitados** (cognitive + tiempo Ricardo ilimitados, **hardware constraints empíricos disco + GPU + RAM + I/O bandwidth finitos**).

**§12 L34 cross-26ª aplicación recursiva al own Sub-Frame 3.A.1 scope**: empirical evidence compute infeasibility hard forcing honest methodology redesign fundamental. Disciplina aspirar-a-lo-mejor literal applied al own Sub-Frame 3.A.1 methodology when empirical evidence emerges — **NO scope reduction subset preserving infeasible methodology** (Opción B/C subset rejected), **MANDATORY methodology redesign fundamental** (Opción F).

**Pattern institutional consolidated cross-Frame 2 → Frame 3 transition reproduced Sub-Frame 3.A.1 internal**: empirical evidence forcing methodology redesign analogous Sesión 4.5 Gate C FAIL → Frame 3 redesign mandatory. Each Sesión empirical evidence emerges forces honest roadmap reformulación.

**Decisión Ricardo strategic Opción F NEW**: Sub-Frame 3.A.1 ARCHIVED_EMPIRICAL + Frame 3.A methodology fundamental redesign mandatory:
- Walk-forward kernel grid sweep ablation matrix methodology REFUTED empíricamente catastrophic.
- Sub-Frame 3.A.1 archived empírico análogo Multi-testing Caso B BH archive 2026-04-26 (refutación hard pre-decisión empirical).
- Sesión 2 Frame 3.A análisis arquitectónica fundamental redesign methodology subsequent — análoga Sesión 1 Frame 2 análisis arquitectónico estructuralmente.

**Refinements arquitectónicos candidatos Sesión 2 Frame 3.A redesign** (preliminary):
...
- Smaller compute footprint kernel implementation refinement (subset of presets, reduced kernel scope).
- Empirical-priori-driven targeted experiments (NOT exhaustive ablation).
- Hipótesis priori H1 architectural coupling validate via reduced experimental approach.
...
**Branch `sesion-1-frame-3a1-2-ablation-matrix-background-launch` preserved git history trazabilidad** (commits `4eea4ec` standalone script implementation + `9d3f156` tests greenfield 12/12 PASS + presets_dir fix) — code potentially reusable Frame 3.A methodology redesign IF refined approach compatible. NO merged main intencionalmente — main at `4eea4ec` (Sub-Frame 3.A.1.1 only) post-archive Sub-Frame 3.A.1 entire scope.

**KILL smoke + cleanup transient**:
- SIGTERM PID 1791 effective (exit 143 = 128+15) — process terminated cleanly.
- `rm -rf sub_frame_3a1_smoke_BTC_baseline/` + log file removed.
- **47 GB transient disk recovered** (vs ~280 GB ahorrados had smoke continued ~9h more).

**Frame 2 methodology refinements infrastructure DEPLOYABLE preserved post-Frame 3.A redesign integrate OR independent reciclaje (escenario F3.3 confirmation)**:
- M2 fix sort + W3 bootstrap CI + W4 thresholds (commit `f8205fa`).
- R3 Path γ kernel granular ASIMÉTRICO TF (6) + MR (8) (commit `d575de4`).
- R1 DSR rigurosa diagnostic flagged_dsr column (commit `71777a9`).
- R4 standalone analytical cross-strategy decomposition (commit `3fe0245`).
- Gate C rigorous analytical zero compute methodology (commit `806cdee`).

**Items §13.3 actualizados**:
- L1296 Frame 3 alcance honesto: Sub-Frame 3.A.1 ARCHIVED_EMPIRICAL + Frame 3.A methodology fundamental redesign mandatory + Sesión 2 Frame 3.A análisis arquitectónica trigger documented.

**Habilita**:
- Sesión 2 Frame 3.A análisis arquitectónica fundamental redesign methodology cuando Ricardo confirme disponibilidad continuación.
- Frame 3.A calendar TBD post-Sesión 2 Frame 3.A análisis redesign.
- Reciclaje launch new calendar TBD post-Frame 3.A redesign + validation gates evaluation rigorous.

**Estado pre-reciclaje FRAME 3.A SUB-FRAME 3.A.1 ARCHIVED_EMPIRICAL + Frame 3.A methodology redesign trigger**: hito histórico cierre Sub-Frame 3.A.1 + apertura Sesión 2 Frame 3.A redesign methodology. Bot v2.4.5 operacional VPS Tokio uptime ~6d 11h+ invariante. Sin tocar `live/*` productivo. Fidelidad 2 invariante por construcción.

**§12 L36 27ª aplicación calibrada empírical catastrophic detection robust**: smoke bg b1euswpru empirical reality cross-9 variants timeline (1h 30 min/9 variants) → projection ~10.7h/cell + ~900h full grid. Empirical refutación ANTES committing ~25 TB cumulative I/O = aspirar-a-lo-mejor literal applied disciplinadamente.

**§12 L38 cross-15ª aplicaciones recursivas validated robust** cross-Frame 2 cumulative + Frame 3.A internal (Parte 0 Sub-fase 3.A.1.0 6 sustantivos + presets_dir mismatch + smoke empirical reality catastrophic): lectura disciplinada código real + escrutinio recursivo cumulative reveala empirical truth pre-commit catastrophic.

**Referencias**:
- Sesión 1 Frame 3.A análisis arquitectónica entrada §13.4 (commit `4d622e9`).
- Sub-fase 3.A.1.1 implementación standalone script entrada (commit `4eea4ec`).
- Branch `sesion-1-frame-3a1-2-ablation-matrix-background-launch` (commit `9d3f156` preserved).
- §12 cross-aplicaciones consolidated robust cross-Frame 2 + Sub-Frame 3.A.1.
- ROADMAP_PRE_RECICLAJE.md (Sub-Frame 3.A.1 ARCHIVED_EMPIRICAL + Frame 3.A redesign trigger).

**Cierre Sub-Frame 3.A.1 ARCHIVED_EMPIRICAL**: permanente. Frame 3.A methodology fundamental redesign mandatory + Sesión 2 Frame 3.A análisis arquitectónica trigger documented. Próxima Sesión 2 Frame 3.A análisis arquitectónica fundamental redesign methodology — análoga Sesión 1 Frame 2 análisis arquitectónico estructuralmente cuando Ricardo confirme.

---

**[ANÁLISIS ARQUITECTÓNICO] [PRE-RECICLAJE FRAME 3.A SESIÓN 1] 6 candidatos γ.1-γ.6 evaluation + priority order refined + Sub-Frame 3.A roadmap detailed — 2026-04-29 sesión post-Sesión 0 Frame 3 metodológica fundamental**

**Contexto**: Sesión 1 Frame 3.A análisis arquitectónica fundamental Claude Code analysis-only post-Sesión 0 Frame 3 commit `d3993a9` + decisiones Ricardo Parte 2 sub-decisiones (ablation matrix mandatory + Opción γ gated-marginal + 5% drift threshold tiered).

**Parte 0 verificación pre-implementación §12 L38**: 0/7 mismatches CLEAN ✓ (predicción §12 L36 24ª acertada lower bound 20-30%). GMM régimen detection methodology current architecture confirmed:
- `train_regime_model.py:51` MAX_K=3 (BIC k∈{2,3}), n_init=10, random_state=42, max_iter=300, covariance_type='full'.
- `regime_features.py:23` 3-feature default `[Hurst, Z_ATR, ER]`. Z_Vol opcional flag OFF default. Cross-symbol features (Z_ATR_BTC) NO implementados (Frame 2 Fase A documented sin landing — 0 hits source código).
- `live/brain_engine.py:92` CONFIRM_THRESHOLD=0.75 fixed all régimens + pairs. Btc override `confidence > 0.80` hardcoded constant separada.
- `regime_walk_forward.py:247-406` episode-based per cluster chronological + toxic_tail dynamic foundation existing pero default fixed=0 legacy.
- Cross-exchange Binance↔BingX H_M6 NO implementado architecturalmente (caveat documentado §13.4 sin validation empírica).

**Parte 1 análisis código real cross-6 candidatos γ.1-γ.6 + tabla scoring refined (Parte 1 → Parte 2)**:

| Candidato | Score Parte 1 → 2 | Justificación refinement |
|-----------|--------------------|--------------------------|
| γ.1 GMM cluster N adaptive | 8/10 → **7/10** | Risk MEDIO-ALTO real (specialist_configs hardcoded cluster IDs + audits + live deployment regenerate pipeline) |
| γ.2 P_threshold adaptive | 7/10 → **6-7/10** | Evidence priori DÉBIL (link Gate C indirecto vs directo γ.1) |
| γ.3 Cross-exchange H_M6 | 5/10 → **5-6/10** | Architectural prereq γ.5 (H2) + observability valuable per se |
| γ.4 Régimen transition | 4/10 → **5/10** | toxic_tail dynamic default activate quick refinement standalone |
| γ.5 Z_ATR_BTC cross-symbol | 5/10 maintained | Frame 2 Fase A honest-flag landing pending (0 hits source) |
| γ.6 HMM/regime-switching | 3/10 maintained | Frame 3.D dedicated last-resort defer (NO within Frame 3.A scope) |

**Parte 2 hipótesis emergentes H1-H3 evaluation**:

- **H1 architectural coupling γ.1 + γ.2 — Sub-Frame 3.A.1 combined**: argumento matemático sustento — N=3 posterior uniform 0.33 + P=0.75 está +0.42 de uniforme; N=6 posterior uniform 0.17 + P=0.75 está +0.58 de uniforme = strict-threshold proporcionalmente más estricto con N higher. γ.1 sin γ.2 introduce strict-threshold hidden regression. Joint testing mandatory + ablation matrix 4 condiciones (baseline + γ.1 only + γ.2 only + Joint).
- **H2 architectural prerequisite γ.3 → γ.5 — Sub-Frame 3.A.3 sequential coupled**: γ.5 Z_ATR_BTC introduce cross-symbol feature Binance offline train → BingX live inference. γ.3 valida drift cross-exchange ANTES γ.5 implementation. Bifurcación crítica tiered: <1% drift confident proceed, 1-5% caveat proceed, ≥5% honest-abandoned o restructured.
- **H3 last-resort defer γ.6 — Sub-Frame 3.A.4 conditional Frame 3.D**: HMM rewrite scope substantial (~5-8 sesiones + compute substantial). Activation only IF Sub-Frame 3.A.1 + 3.A.2 + 3.A.3 cumulative FAIL Gate C rigorous re-evaluation. NOT within Frame 3.A scope.

**Sub-Frame 3.A roadmap detailed**:

- **Sub-Frame 3.A.1** (γ.1 + γ.2 H1 combined): scope ~3-4 sesiones cumulative ~12-18h CC + ~30-60h compute background. Sub-fases: 3.A.1.0 Parte 0 + 3.A.1.1 standalone retrain script grid 21 cells × 3 sym = 63 runs + 3.A.1.2 ablation matrix mandatory 4 condiciones + 3.A.1.3 tests greenfield + 3.A.1.4 validation gates + 3.A.1.5 commit. Decision rule Opción γ post-Sub-Frame 3.A.1: STRONG signal ρ_mean > 0.4 early-exit / marginal [0.3, 0.4] proceed reinforcement / FAIL Sub-Frame 3.A.4 conditional.
- **Sub-Frame 3.A.2** (γ.4 toxic_tail dynamic default): scope ~1-2 sesiones cumulative ~4-8h CC + ~5-10h compute ablation. Sub-fases: 3.A.2.0 Parte 0 + 3.A.2.1 activation default + 3.A.2.2 walk-forward ablation + 3.A.2.3 validation gates + 3.A.2.4 commit.
- **Sub-Frame 3.A.3** (γ.3 + γ.5 H2 sequential coupled): scope ~3-4 sesiones cumulative ~12-18h CC + ~15-30h compute moderado. Sub-fases: 3.A.3.0 Parte 0 + 3.A.3.1 γ.3 cross-exchange validation + bifurcación drift threshold tiered + 3.A.3.2 γ.5 implementation CONDITIONAL + 3.A.3.3 tests + validation gates + 3.A.3.4 commit.
- **Sub-Frame 3.A.4** (γ.6 HMM CONDITIONAL Frame 3.D defer): activation criterion Sub-Frame 3.A.1 + 3.A.2 + 3.A.3 cumulative FAIL. Scope ~5-8 sesiones cumulative ~25-40h CC + +substantial compute. NOT within Frame 3.A scope.

**Frame 3.A cumulative scope total refined**:
- Sub-Frames 3.A.1 + 3.A.2 + 3.A.3 = ~7-10 sesiones cumulative ~28-44h CC + ~50-100h compute background.
- Sub-Frame 3.A.4 conditional = +5-8 sesiones cumulative if needed.
- Reciclaje launch new calendar TBD post-Frame 3.A conclusión empírica.
...
**Decision rule Opción γ gated-marginal documented explícito** (Ricardo confirmed Parte 2):
- Sub-Frame 3.A.1 ejecuta primero + Gate C rigorous re-evaluation.
...
**Ablation matrix mandatory Sub-Frame 3.A.1** (Ricardo confirmed Parte 2):
- 4 condiciones: baseline N=3 P=0.75 + γ.1 only (N∈{4,5,6} P=0.75 fixed) + γ.2 only (N=3 P adaptive grid) + Joint (N∈{4,5,6} P adaptive).
- Honest disambiguation FAIL atribución per factor + §12 L26 disciplina validación per-componente applied.

**Drift threshold tiered interpretation Sub-Frame 3.A.3** (Ricardo confirmed Parte 2):
- <1% drift: proceed γ.5 confident.
- 1-5% drift: proceed γ.5 with caveat documented.
- ≥5% drift: γ.5 honest-abandoned o restructured (BingX-only training).
- Permite empirical evidence validate threshold robust o calibrate refinement Sub-Frame 3.A.3 Parte 0 verification.

**§12 disciplinas cross-aplicaciones consolidated**:
- **§12 L34 cross-22ª aplicación recursiva al own roadmap propio applied**: scope reduction 6 candidatos individuals → 3 Sub-Frames consolidated + 1 conditional Sub-Frame 3.A.4. Disciplina aspirar-a-lo-mejor literal applied al own roadmap when evidence emerges Parte 1 análisis comprehensive.
- **§12 L36 24ª aplicación calibrada acertada lower bound** (predicción 20-30% mismatches → real 0/7 CLEAN) + Parte 0 verification mandatory cada Sub-Frame.
- **§12 L37 V2 calibrated honest**: scope estimates 1.5-2x baseline applied post-Sesión 2.5 + 3 + 3.5 cumulative refutados. Sub-Frame 3.A.1 grid sweep ~30-60h compute background honest framing.
- **§12 L38 disciplinada cross-iteraciones validated robust** cross-Frame 2 cumulative + cross-Frame 3.A análisis: lectura disciplinada código real + escrutinio recursivo cumulative.

**Items §13.3 actualizados**:
- L1296 Frame 3 alcance honesto: Sesión 1 Frame 3.A análisis arquitectónica done + Sub-Frame 3.A.1 priority#1 (combined γ.1+γ.2 H1 architectural coupling) trigger documented.

**Habilita**:
- Sub-Frame 3.A.1 sub-sesiones subsequent cuando Ricardo confirme disponibilidad continuación.
- Frame 3.A calendar TBD post-Sub-Frames 3.A.1 + 3.A.2 + 3.A.3 conclusión empírica + decision rule Opción γ gated-marginal.

**Estado pre-reciclaje FRAME 3.A INICIO + Sesión 1 análisis arquitectónica fundamental done**: bot v2.4.5 operacional VPS Tokio uptime ~6d 8h+ invariante. Sin tocar `live/*` productivo. Fidelidad 2 invariante por construcción.

**Próxima Sub-fase 3.A.1.0 Parte 0 verificación pre-implementación combined γ.1 + γ.2 retrain + ablation matrix mandatory** cuando Ricardo confirme disponibilidad continuación.

**Referencias**:
- Sesión 0 Frame 3 metodológica fundamental entrada §13.4 (commit `d3993a9`).
- §13.2 walk-forward methodology bloque (Frame 3.A análisis update).
- §12 cross-aplicaciones consolidated robustas Frame 2 cumulative applicable Frame 3.
- ROADMAP_PRE_RECICLAJE.md (Frame 3.A roadmap detailed Sub-Frames + decision rule Opción γ).

**Cierre Sesión 1 Frame 3.A análisis arquitectónica fundamental**: permanente. Sub-Frame 3.A.1 priority#1 trigger documented. Próxima Sub-fase 3.A.1.0 Parte 0 verificación.
...
**Contexto**: Sesión 4.5 Frame 2 R4 empirical subset reduced (DEFERRED §12 L37 V2 cumulative cross-3 intentos previos refutados — precalculate_all_data upstream cost dominante NO escapable bg mode) + Gate C rigorous analytical refinement (zero compute cost cross-clusters Spearman ρ from JSONs metadata directly) + decisión Ricardo strategic post-Sesión 4 analytical proxy reveal caveats sustanciales. Disciplina aspirar-a-lo-mejor literal applied al own Frame 2 conclusion via Sesión 4.5 rigorous refinement.

**Hallazgo crítico Sesión 4.5 Gate C rigorous analytical**:

Per-cluster Spearman ρ(pf_tr, pf_fwd) cross-9 BTC+ONDO+SEI × C0+C1+C2:

| Sym | Cluster | ρ | p-value | Stable (|ρ|≥0.3) | Significant | Sign |
|-----|---------|---:|--------:|:---:|:---:|:---:|
| BTC | C0 | +0.185 | 0.0653 NS | ✗ | ✗ | + |
| BTC | C1 | -0.004 | 0.9680 NS | ✗ | ✗ | – |
| BTC | C2 | +0.013 | 0.8968 NS | ✗ | ✗ | + |
| ONDO | C0 | +0.221 | 0.0273 * | ✗ | ✓ | + |
| ONDO | C1 | +0.044 | 0.6673 NS | ✗ | ✗ | + |
| ONDO | C2 | **-0.502** | 0.0000 *** | ✓ | ✓ | **–** |
| SEI | C0 | +0.127 | 0.2075 NS | ✗ | ✗ | + |
| SEI | C1 | **-0.363** | 0.0002 *** | ✓ | ✓ | **–** |
| SEI | C2 | +0.046 | 0.6515 NS | ✗ | ✗ | + |

Cross-9 aggregates:
- Mean ρ: **-0.026** (~null).
- N stable (|ρ|≥0.3): **2/9** ambos NEGATIVE (over-fit phenomenon counter-intuitive).
- N significant: 3/9.
- **N stable + positive (Gate C strict criterion ≥+0.30 sig): 0/9 → FAIL**.
- Sign reversals: 3/9 → instability cross-clusters confirmed.

**Hallazgo metodológico significativo**:

- **7/9 clusters: ρ ≈ 0** → train pf NO predice fwd pf within survivor pool top-100 (selection bias structural confirmado empíricamente per-cluster cross-9).
- **2/9 clusters: ρ STRONG NEGATIVE** (ONDO C2 -0.502 *** + SEI C1 -0.363 ***) → counter-intuitive over-fit phenomenon (top train ranks have inverted fwd ranks).
- M2 fix mejoró absolute magnitude J/B (Gate A 2.408 ≤ 3.62 + Gate B 0/9 colapso fuerte > 5.0 PASS) PERO ranking selectivity NO mejorada empíricamente per-cluster.
- Confirma §12 L29 + §13.2 walk-forward methodology bias structural cluster-level rigorous.

**Re-evaluación Frame 2 escenarios post-Sesión 4.5 rigorous**:

| Escenario | Probability original | Post-Sesión 4.5 rigorous |
|-----------|---------------------:|--------------------------|
| 1 (PASS+PASS+PASS) | 30-40% | **REFUTED** empirically |
| 2 (Gate A FAIL, B+C PASS) | 40-50% | NO applicable (Gate A PASS, Gate C FAIL) |
| **3 (Gate B/C FAIL)** | 15-25% | **DETECTED EMPIRICALLY ROBUST** ✓ |

**Decisión Ricardo strategic Opción A: Frame 3 redesign mandatory + reciclaje launch DEFERRED**.

Razones consolidadas honestas:
- **§12 L34 disciplina aspirar-a-lo-mejor literal applied al own Frame 2 conclusion**: Sesión 4 analytical proxy gave optimistic verdict PARTIAL. Sesión 4.5 rigorous (analytical, zero compute cost) reveals empirical FAIL strict criterion. Honest scrutiny prevents Frame 2 launch on flawed Gate C foundation.
- **Frame 2 spec L1296 design**: "FAIL Gate B/C → pause + diagnóstico arquitectónico §12 L34". Triggered directly por Gate C rigorous FAIL.
- **Pattern Frame 1 implícito rejection**: Opción B reciclaje + R5 monitoring post-launch reproduce "patada hacia delante" que Frame 2 metodología fundamental Sesión 0 designed evitar.
- **Capital scale-up bajo Gate C FAIL = riesgo asimétrico downside-amplified rejected**.
- **Opción C re-interpretation criterion to fit desired outcome rejected** disciplina aspirar-a-lo-mejor.

**§12 L36 23ª aplicación retrospectiva calibrada empírica robusta**: Sesión 4 analytical proxy gave Escenario 1 PARTIAL → Sesión 4.5 rigorous reveals Escenario 3 detected empirically robust. Probability Sesión 1 análisis Parte 3.4 escenarios 3-way documented preserved honest. Cross-23 aplicaciones consolidada (~58-100h ahorro acumulado paths infeasibles ciegamente).

**§12 L37 V2 cumulative cross-aplicaciones validated robusta** (Sesión 2.5 + Sesión 3 + Sesión 3.5 cross-3 intentos refutados): precalculate_all_data upstream cost dominante NO escapable bg mode. R4 empirical Sesión 4.5 DEFERRED full reciclaje natural compute OR Frame 3 redesign arquitectura full pipeline cost integrated.

**§12 L34 cross-20ª aplicación recursiva al own Frame 2 conclusion robust**: Frame 2 alcance honesto explícito documented Sesión 0/1 "refinements pre-reciclaje NO mejoran edge real disponible mercado — atacan sobrestimación métricas walk-forward para selección honesta configs reciclaje" confirmado empíricamente. Refinements R1+R3+R4 done infrastructure deployable + Gate C FAIL reveals walk-forward ranking selectivity within survivor pool unchanged.

**§12 L38 cross-13ª aplicaciones recursivas validated**: lectura disciplinada código real + escrutinio recursivo cumulative cross-Frame 2 patrón institucional consolidado robusto. Sesión 4.5 rigorous analytical (zero compute) refines Sesión 4 analytical proxy → empirical Gate C FAIL.

**Frame 2 conclusion honest robust**:

- **Methodology refinements infrastructure DEPLOYABLE post-Frame 3 redesign integrate**:
  ...
  - R1 DSR rigurosa diagnostic flagged_dsr column (Sesión 2.5 Opción C standalone).
  - R4 standalone analytical cross-strategy decomposition (Sesión 3 Sub-fase 3A).
- **Walk-forward ranking selectivity within survivor pool unchanged empíricamente** — Frame 3 redesign requires **architectural changes fundamental** (NO walk-forward methodology refinement scope).

**Items §13.3 actualizados**:
- L1296 Frame 2 alcance honesto: Gates A+B+C cross-9 evaluation done Sesión 4 + Sesión 4.5 rigorous → Escenario 3 detectado empíricamente → Frame 3 redesign mandatory + reciclaje launch DEFERRED.

**Habilita**:
- **Frame 3 redesign session subsequent dedicated** arquitectura análisis fundamental: edge mejora real architectural changes (NO walk-forward methodology refinement scope) + cross-strategy ensemble combinaciones R5 L162 + revisión fundamental régimen detection methodology + scope ~10-30h CC + compute background substantial estimated.
- **Calendar Frame 2 reciclaje launch ~2026-05-03 SUSPENDED** → Frame 3 redesign session subsequent + new calendar TBD post-Frame 3 redesign analysis arquitectónica fundamental.

**Estado pre-reciclaje FRAME 2 CONCLUSION + transición Frame 3 redesign**: bot v2.4.5 operacional VPS Tokio uptime ~6d 5h+. Sin tocar `live/*` productivo (analytical refinement read-only sobre JSONs). Fidelidad 2 invariante por construcción.

**Próxima Frame 3 redesign session**: análisis arquitectónico fundamental edge mejora real architectural changes (alcance fundamental diferente Frame 2) cuando Ricardo confirme disponibilidad continuación.

**Referencias**:
- `analysis_scripts/gate_c_rigorous_analytical.py` (Sesión 4.5 rigorous script).
- `gate_c_rigorous_analytical_results.json` (output JSON).
- §13.4 entrada Sesión 4 Gates evaluation 2026-04-29 (analytical proxy).
- §13.2 walk-forward methodology bloque (Frame 2 conclusion update).
- §12 L29 + L34 + L36 + L37 V2 + L38 cross-aplicaciones consolidadas robustas.
- Frame 2 spec L1296 design (R5 condicional Gate B/C FAIL trigger).

**Cierre Sesión 4.5 Frame 2 CONCLUSION + Frame 3 trigger**: permanente. Frame 2 methodology refinements infrastructure deployable + Frame 3 redesign mandatory architectural changes fundamental scope. Honest aspirar-a-lo-mejor literal applied al own Frame 2 conclusion via empirical scrutiny.
